...
test('ship workflow and durable state share one closed held-reason contract', () => {
  ...
  assert.equal(contract.ship.trusted_host_executor, 'implemented_external_workflow')
  assert.equal(contract.ship.trusted_host_contract, './trusted-host-merge.contract.json')
  assert.equal(contract.ship.trusted_host_workflow, '.github/workflows/trusted-elevated-merge.yml')
  assert.equal(contract.ship.dispatch_protocol, 'github-workflow-dispatch')
  assert.equal(contract.ship.activation_state, 'requires_live_attestation')
  assert.equal(contract.ship.unavailable_reason, 'trusted_elevated_authorization_unavailable')
})
