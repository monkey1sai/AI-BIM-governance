...
- `run_blip_live_approve_once.ps1`: deterministic fixed-User broker for a counted GitHub `APPROVE`. Its byte-exact body is a distinct `ai-bim-automated-approve-only` schema with `automated=true` and `action=approve-only`; it cannot satisfy trusted `merge` or `merge-elevated` authority. It remains separate from all model execution and requires an exact canonical SHIP attestation. It also requires a freshly owner-minted protection attestation at `<product root>\secrets\blip-protection-attestation.v1.txt` plus the dedicated key at `<product root>\secrets\blip-protection-attestation-key.v1.txt`; only the pinned child receives them as `BLIP_PROTECTION_ATTESTATION` and `BLIP_PROTECTION_ATTESTATION_KEY`.
- `mint_protection_attestation.py`: owner-side protection evidence courier. The fixed reviewer is pinned to least-privilege write permission and receives HTTP 404 from GitHub's admin-only legacy protection endpoint (verified 2026-09-01), so the owner mints a signed protection snapshot with a separate read-only admin-scope credential and a dedicated attestation HMAC key—not the counted-reviewer PAT. The broker re-validates every policy field on the embedded snapshot itself and additionally cross-checks the write-visible GraphQL `refUpdateRule` live on every preflight and post-approval pass. Re-mint immediately before every approval; both age and validity are capped at ten minutes.
...
4. Token-bearing Python children are limited to collector, binder, and poster. The model-gate Python process receives no GitHub token/App identity environment variables; its version-pinned Codex children run with an explicit allow-none tool feature contract, and both the model runner and poster reject outbound secret/DLP signatures fail-closed.
...
& 'C:\Program Files\Python312\python.exe' -I -S -B scripts\agent-tooling\blip-approve\bot\scripts\test_mint_protection_attestation.py
