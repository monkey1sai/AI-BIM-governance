...
Branch protection is verified from two sources, because the least-privilege write reviewer receives HTTP 404 from GitHub's admin-only legacy protection endpoint (verified 2026-09-01 across ten failed vote attempts): the owner mints a signed protection snapshot with `mint_protection_attestation.py` using a separate read-only admin-scope credential, the wrapper reads it from `<product root>\secrets\blip-protection-attestation.v1.txt` and injects it as `BLIP_PROTECTION_ATTESTATION`, and `blip_review.py` re-validates every policy field on the embedded snapshot plus cross-checks the write-visible GraphQL `refUpdateRule` live on every preflight and post-approval pass. A missing, malformed, unsigned, expired, or drifted attestation fails closed before any mutation; the attestation signature reuses the fixed reviewer token as the shared HMAC secret, matching the approval-capability idiom.
