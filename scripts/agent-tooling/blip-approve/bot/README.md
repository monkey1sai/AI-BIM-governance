...
The broker reads the reviewer credential only from fixed `C:\Users\IOT\.grok\github-bot\.env.blip` during separately authorized live use. It opens the file exclusively after verifying the immutable owner SID, protected non-inherited ACL, explicit sandbox denial, regular-file/non-reparse path, bounded strict UTF-8 content, and exactly one `BLIP_GITHUB_TOKEN` assignment. Agents, editable source, and offline tests never read either real credential. A successful live call must verify GitHub response plus readback parity for review ID/URL/User/state/body/commit. Before installation, the external protected verifier resolves the namespaced `heads/main` commit using redirect-free, strict-UTF-8, byte-bounded public reads with an independent body deadline. It uses bounded commit-to-pulls pagination to locate an exact `head.sha == source_commit` candidate, fetches the full PR detail because the simplified associated-pulls response may expose `merged_by=null`, and requires exactly one completed `main` merge by fixed owner `monkey1sai` (User `26239865`). The latest decisive `monkey1sai-blip` (User `311287868`) review must be a counted `APPROVED` submitted before merge on that exact source head. The verifier then re-reads `heads/main`, rejects snapshot drift, and proves the PR's merge commit is reachable from the unchanged SHA. Review body text is not installation authority because GitHub permits it to be edited without changing the original `submitted_at`; the broker's canonical body remains audit/output metadata. This prevents an admin push or temporary protection bypass from activating broker code that skipped counted review; the installed manifest preserves the source commit, and the broker rejects any vote whose expected PR head equals it. A candidate runtime may be activated only after its source has merged through an independent counted approval; it cannot approve the PR that introduced itself.
...
The installer publishes a manifest-bound runtime under the protected product root. Runtime code/state is read/execute-only to the sandbox with write/delete/ACL/ownership denied; Codex login state and secrets are owner-only with sandbox access denied. An existing `v1` can be replaced only from the exact predecessor tuple pinned by the reviewed installer. One exclusive product lock serializes initial install, upgrade, and recovery. The upgrade fences active readers, denies fixed-owner read/execute on the quiesced predecessor before releasing those fences, and requires changed User/App generation-pivot hashes plus strict manifest/completion v2 so a cached v1 wrapper cannot execute against successor bytes. The write-through journal binds the target source, manifest, and freeze before publication. Mutable directories move without widening `codex-home`. A protected temporary completion payload is atomically renamed to the final marker as the irrevocable activation commit point: earlier failures restore the verified predecessor or quarantine an incomplete initial root; a present ambiguous marker remains fail-closed; and an exact committed target is never rolled back because of later health drift or journal archival interruption. Previous and failed roots are retained. A future runtime requires a separately reviewed predecessor-tuple update.

Candidate output is inert data. Production construction starts only through the separately protected builder launcher and an independently authorized reviewed-manifest hash. The v3 freeze binds a reviewed-build v2 manifest and clean source commit; the candidate excludes the builder, both launchers, tests, and `invoke_protected_blip_installer.ps1`. Installation starts only through the separately protected public installer launcher in an exact fixed-PowerShell `-NoProfile -NonInteractive -File` process. The internal verifier has no bypass phase, validates manifest authority and pinned launcher/verifier/bootstrap provenance, proves the source was the exact head of one fixed-owner merged PR with a pre-merge fixed-reviewer counted approval, and proves that PR's merge commit is reachable from an unchanged namespaced protected-main snapshot before bootstrap. The bootstrap rejects any freeze source/runtime tuple or installer command outside the reviewed contracts, and the installer records the verified source commit in the installed manifest. Test-only freezes carry `build_profile=TEST_ONLY` plus zero provenance sentinels and are rejected by both bootstrap and installer.
