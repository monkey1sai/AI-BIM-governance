...
function Stage-PinnedMarketplace {
 ...
 $psi=[Diagnostics.ProcessStartInfo]::new(); $psi.FileName=$PwshExecutable; $psi.WorkingDirectory=$stage; $psi.UseShellExecute=$false; $psi.RedirectStandardOutput=$true; $psi.RedirectStandardError=$true; @('-NoProfile','-NonInteractive','-File',$MarketplaceScript,'-RequestFile',$request,'-ResultFile',$result)|ForEach-Object {[void]$psi.ArgumentList.Add($_)}; $psi.Environment.Clear(); foreach($e in $envs.GetEnumerator()){$psi.Environment[$e.Key]=$e.Value}; $p=[Diagnostics.Process]::Start($psi); $p.WaitForExit(); if($p.ExitCode -ne 0){throw "Marketplace child failed (exit $($p.ExitCode)): $($p.StandardError.ReadToEnd())"}; if(-not (Test-Path $result)){throw 'Marketplace child did not produce result JSON'}; $child=Get-Content -Raw $result|ConvertFrom-Json; if([string]$child.status -ne 'ok' -or -not $child.stagedRepo){throw 'Marketplace child result is invalid'}
