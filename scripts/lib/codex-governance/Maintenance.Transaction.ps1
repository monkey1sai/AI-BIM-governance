...
function Resume-InterruptedTransaction {
 ...
 $d=$j.Data; if(-not $d -or -not ($d.PSObject.Properties.Name -contains 'SnapshotPath') -or -not ($d.PSObject.Properties.Name -contains 'TargetPath') -or -not [IO.Path]::IsPathRooted([string]$d.SnapshotPath) -or -not [IO.Path]::IsPathRooted([string]$d.TargetPath)){throw 'Invalid journal Data: absolute SnapshotPath and TargetPath required'}; if(-not (Test-Path $d.SnapshotPath)){throw 'Missing snapshot; recovery aborted'}; Resolve-ContainedPath -Root $CodexHome -Path $d.SnapshotPath | Out-Null; Resolve-ContainedPath -Root $CodexHome -Path $d.TargetPath | Out-Null; Copy-Item $d.SnapshotPath $d.TargetPath -Force
