...
function Expand-ValidatedArchive {
 ...
 $Destination=[IO.Path]::GetFullPath($Destination); $destParent=Split-Path $Destination -Parent; if(Test-Path $Destination){Assert-SkillPath $Destination $Destination|Out-Null} else { Assert-SkillPath $destParent $Destination|Out-Null }; $root=$Destination.TrimEnd('\')+'\'; $seen=@{}; $zip=[IO.Compression.ZipFile]::OpenRead($ArchivePath)
