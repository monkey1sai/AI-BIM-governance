...
function Stage-CliClosure {
 ...
 $actual=Get-ContentTreeHash $Staging; [pscustomobject]@{Prefix=$Prefix;Staging=$Staging;Version=$c.Version;ClosureHash=$c.ClosureHash;TreeHash=$actual;OldVersion=$OldVersion;NewVersion=$NewVersion}
}
...
function Test-CliLifecycleBaseline {
 ...
 if($ml -and $cl -and $ml -ne $cl){throw 'Lifecycle-script change is not allowed'}
...
function Invoke-CliRollbackRehearsal {
 ...
 $before=Get-ContentTreeHash $Prefix; $tmp=Join-Path (Split-Path $Prefix -Parent) ('.rehearsal-'+[guid]::NewGuid().ToString('N')); Copy-Item $Prefix $tmp -Recurse -Force; try { Remove-Item $Prefix -Recurse -Force; Rename-Item $tmp (Split-Path $Prefix -Leaf); $after=Get-ContentTreeHash $Prefix; if($before -ne $after){throw 'Rollback rehearsal hash mismatch'}; [pscustomobject]@{Status='pass';CurrentVersion=$CurrentVersion;CandidateVersion=$CandidateVersion;BeforeHash=$before;AfterHash=$after} } finally {if(Test-Path $tmp){Remove-Item $tmp -Recurse -Force}}
}
