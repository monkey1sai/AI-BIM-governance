...
function Get-ContentTreeHash {
 ...
 $reparseChild=Get-ChildItem -LiteralPath $Root -Force -Recurse -ErrorAction Stop | Where-Object { ($_.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0 } | Select-Object -First 1
 if($reparseChild){ throw "Reparse point not allowed: $($reparseChild.FullName)" }
