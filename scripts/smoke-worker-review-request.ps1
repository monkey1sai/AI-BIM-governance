...
# 本 smoke 原以兩 mock 服務為核心，已不可運作。B-scheme 的 default smoke 不依賴
# _worker/_bim-control，改以 tests/contracts + tests/fakes 對 coordinator 對外
# intake / 雲端 callback outbox 驗證（OpenSpec change T8；契約：
# tests/contracts/ifc_ready_payload.json、tests/contracts/conversion_result_callback.json）。
