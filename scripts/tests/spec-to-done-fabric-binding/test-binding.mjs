...
const providerSession = (overrides = {}) => ({
  ...
  resource_keys: [
    ...
    'scope:spec-to-done-library',
    'contract:spec-to-done-binding',
  ],
...
test('allowed paths outside explicit path/glob authority remain scope_drift', () => {
  ...
  const lease = sessionLease({ scope_digest: scopeDigest })
  const session = providerSession({ scope_digest: scopeDigest })
  assert.throws(
