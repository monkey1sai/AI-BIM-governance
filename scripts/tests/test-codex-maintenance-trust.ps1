...
try { $allow=@{sources=@(@{id='src';kind='git';uri='https://example.invalid/repo';refPolicy='pinned'})}; $allowPath=Join-Path $root 'allow.json'; $seal=Join-Path $root 'allow.json.seal'; $allow|ConvertTo-Json -Depth 10|Set-Content $allowPath -Encoding utf8; Seal-Allowlist $allowPath $seal|Out-Null; Read-SealedAllowlist $allowPath $seal | Out-Null
 $base=@{sourceId='src';kind='git';uri='https://example.invalid/repo';refPolicy='pinned';subpaths=@();validatorIds=@('v1');cohort='stable';cliRange='>=1';independent=$true;nativePaths=@('C:\tool');capabilityBaseline=@{a=1}}
