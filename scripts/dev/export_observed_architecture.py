#!/usr/bin/env python3
"""Export the observed architecture graph and enforce the ratchet against the baseline."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Statically scan committed source into a deterministic observed dependency report "
            "and compare it against architecture/observed-baseline.json."
        )
    )
    parser.add_argument(
        "--repo-root",
    ...
    parser.add_argument(
        "--report-only",
        action="store_true",
        help="Write the observed report without applying the baseline ratchet.",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"ERROR: could not import scripts.lib.observed_architecture from {repo_root}: {exc}",
            file=sys.stderr,
        )
        return 2

    if args.report_only:
        config, config_issues = _load_config(repo_root)
        blocking = [issue for issue in config_issues if issue.severity == "error"]
        if config is None or blocking:
            for issue in blocking:
                print(f"[ERROR] {issue.code} {issue.path}: {issue.message}", file=sys.stderr)
            return 1
        rendered = render_report_json(build_observed_report(repo_root, config))
        _emit(rendered, args.output, repo_root)
        return 0

    result = check_observed_architecture(repo_root)
    if args.format == "json":
        rendered = json.dumps(result.to_dict(), indent=2, ensure_ascii=False, sort_keys=True) + "\n"
    else:
        report = result.report
        lines = [
            f"Observed architecture ratchet: {result.status.upper()}",
            f"Repository: {result.repo_root}",
            f"Scanned files: {report.scanned_file_count if report else 0}",
            f"Service edges: {len(report.edges) if report else 0}; "
            f"cycles: {len(report.cycles) if report else 0}",
