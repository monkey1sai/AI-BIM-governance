...
- MUST 在 reconcile 新增或改寫 lifecycle row 時同時宣告 `subject_binding: "introduction"`；squash 後懸空的 subject 由 introduction recovery 自癒，不另開 rebind PR，rebind 僅作為 recovery fail-closed 時的修復手段。慣例正本：`docs/agents/agent-governance-policy.md`（Lifecycle-ledger subject binding）；規格正本：`openspec/changes/introduction-resolved-subject-binding/`。
