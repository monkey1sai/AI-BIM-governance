...
- MUST 在 reconcile 新增或改寫 lifecycle row 時同時宣告 `subject_binding: "introduction"`；squash 後懸空的 subject 由 introduction recovery 自癒，不另開 rebind PR，rebind 僅作為 recovery fail-closed 時的修復手段。慣例正本：`docs/agents/agent-governance-policy.md`（Lifecycle-ledger subject binding）；規格正本：`openspec/specs/openspec-lifecycle-ledger-schema/spec.md`、`openspec/specs/openspec-machine-truth-subject-resolution/spec.md`、`openspec/specs/openspec-machine-truth-reconcile-ratchet/spec.md`（archive：`openspec/changes/archive/2026-08-20-introduction-resolved-subject-binding/`）。
...
- MUST 將既有歷史 archive 的 unchecked／unsupported checkbox 只記為 typed `archive_debt`；這是 migration debt disclosure，不是完成證據，也不得用於新的 archive。兩種變體：`historical_task_ledger_debt`（owner + review_due，待裁決）與 `permanent_historical_task_ledger_debt`（owner + adjudicated_on，owner 已裁決永久不回補——2026-08-17 依 issue #551 R2 整批改判）。permanence 改判只能在 counts 與 owner 不變下由 historical 變體轉入，不得憑空新增。
