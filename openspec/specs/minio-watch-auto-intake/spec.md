...
> Deferred archive（2026-07-22，`--skip-specs`）：原 `minio-folderview-and-baseline-disclosure` change 的 ledger 去重與手動觸發提案保留在 `openspec/changes/archive/2026-07-22-minio-folderview-and-baseline-disclosure/`，但未合併至 canonical specs。因此 archive 內容僅為 non-canonical 歷史工件，**不** supersede 本 requirement；若要重啟，MUST 先依當時 main 現況新開／調和 change，並重新驗證 watcher／intake contract。
