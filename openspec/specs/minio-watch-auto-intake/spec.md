...
> 被 supersede（pending archive）：`openspec/changes/minio-folderview-and-baseline-disclosure/`（使用者 2026-06-24 拍板：全自動 auto-enroll）把 watcher tick 去重由「首輪 SHALL 只登記 baseline 不觸發、之後新 key/新 etag 才觸發」改為**持久 ledger 去重**——每輪對命中物件算 `idempotencyKeyFor(bucket,key,etag)` 查持久 ledger，**無紀錄才觸發並落帳、有紀錄 skip**（即「ledger 無紀錄才觸發」取代「首輪 baseline 不觸發」）；既有未轉檔下一輪自動補轉，重啟重掃命中既有 ledger 紀錄即 skip（重啟不風暴，去重水印即既有持久 ledger、非新建 watermark）。`deriveIntakeFromKey`/`idempotencyKeyFor`/ledger schema 不改。同時推翻 closed-loop design 非目標「不新增手動觸發 UI」（新增 `POST /api/conversion/trigger` 一鍵手動觸發鈕）。權威以該 change spec delta `specs/minio-watch-auto-intake/spec.md` 為準；本段於該 change archive 後由 `npx openspec archive` 落地覆寫。另：本檔下方「已知限制（v1…seen 不持久化、停機期間上傳被 baseline 吸收）」一段亦由此 change 解除（停機期間上傳因 ledger 無紀錄仍會自動觸發）。
