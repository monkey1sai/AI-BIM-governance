# openspec-machine-truth-subject-resolution Specification

## Purpose
sentinel row 的有效 subject 先走 self-anchored（`subject_commit` 是 HEAD ancestor），否則以 introduction-recovery 解析唯一 introduction commit。解析失敗一律 HELD；legacy row 三分支行為不變。
## Requirements
