# Change: 以 docs/plans HTML 重建前端 design authority 與 dual gate
...
- 由 changed paths 與 base/head 的 HTML-derived scope 聯集機器推導 product `passed`／`mixed`／`partial_reference_missing`，以及 non-product `design_source_update_only`／`gate_infrastructure_only`；同一 change 同時修改 authority HTML與production UI時為 `design_source_and_product_mixed_fail_closed`。Unknown、source drift、無法回溯至 HTML 或缺少衍生 artifacts 時 fail closed。
