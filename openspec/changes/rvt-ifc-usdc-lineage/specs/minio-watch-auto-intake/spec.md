...
> Governed 邊界（`rvt-ifc-usdc-lineage` 新增）：本 requirement 描述的觸發鏈固定為 **legacy intake**。watcher 對 `MINIO_WATCH_KEY_SUFFIX`（預設 `/model.ifc`）命中的 object observation SHALL 只建立既有 `POST /api/external/ifc-ready` intake job，MUST NOT 產生、推導或宣告 governed source-bundle `READY`。Governed `READY` 只能由最後發布並通過 integrity 驗證的 source `manifest.json` 產生（由 `minio-model-version-bundle` 擁有）；watcher SHALL NOT 以 `manifest.json` 作為觸發後綴，也 SHALL NOT 把 `mw_<hash16>` ledger 紀錄當成 `source_bundle_id`、`pipeline_job_id` 或任何 governed lineage identity。Dual-read 期間 watcher ledger 與 governed `source_bundle_id` SHALL 是兩個各自獨立的去重空間，MUST NOT 互相取代或互相抑制；legacy intake 產生的 conversion 結果 MUST NOT 進入 governed active-result pointer，也 MUST NOT 觸發 `cloud-lineage-publication` 的任何 event。Governed lineage publication 的 authority 由 `cloud-lineage-publication` 擁有，本 capability 不擁有。本段只宣告邊界，上述既有輪詢、去重、重試與 malformed 判定行為 SHALL 逐字不變。
...
#### Scenario: watcher 觸發不得冒充 governed READY

- **WHEN** watcher 對命中 `/model.ifc` 後綴的 object 觸發 loopback `POST /api/external/ifc-ready`
- **THEN** 產生的 job SHALL 只是 legacy intake，SHALL NOT 被標為 governed source-bundle `READY`
- **AND** 系統 SHALL NOT 由該 object observation 推導 `source_bundle_id` 或建立 governed `pipeline_job_id`
- **AND** 同一 version prefix 若另有通過 integrity 驗證的 source `manifest.json`，governed `READY` SHALL 只由該 manifest 裁決
