...
- [ ] 2.6 將HMAC raw-body canonicalization、canonical unsigned decimal Unix-seconds timestamp header／非canonical格式拒絕、±300秒skew、header/body event match、201/200 ACK與malformed/202 protocol failure建立contract tests；表驅動驗證九個canonical HTTP/code/retryable三元組與至少兩個status/code cross-swap，另驗證pre-validation error可不帶`event_id`、若帶則必須是有效UUID。Reference MySQL 8 DDL保持醒目`REFERENCE ONLY`，不得作repo migration。
...
- [ ] 7.4 實作`POST /api/v1/lineage-publications` HMAC raw-body client與strict 200/201 exact ACK；signature timestamp只接受canonical ASCII unsigned decimal Unix epoch seconds，exact header string參與簽章且receiver不得normalize；receiver transaction先建立／檢查全域immutable event ledger，同event/different tuple回409，publication first event、每筆health與receipt皆逐欄綁定event tuple，receipt另綁publication identity/digest/registration ACK tuple；published另以RFC 8785 JCS projection計算`publication_content_sha256`，只有identity／manifest／content digest全同才replay；實作canonical HTTP/code/retryable三元組，202/empty/mismatched 2xx或status/code mismatch不得標DELIVERED。
...
- [ ] 7.6 擴充test-only cloud fake與contract/integration tests，模擬201 commit、200 replay、bad HMAC、signature timestamp的leading-zero／sign／fraction／milliseconds／RFC3339／out-of-window拒絕、invalid edge authority字元、ambiguous／mismatched publication identity、同event ID異event type／publication／raw body 409、publication／health direct-import缺少或不符ledger tuple、同identity／manifest異canonical content 409、403 tenant mismatch、422 missing parent、九個canonical error三元組與status/code cross-swap、malformed ACK、lost ACK/restart、late older health event、equal-observed-at tie-break、offset／sub-microsecond／leap-second／out-of-range observed-at rejection、health restore/tombstone、non-tombstone tombstone ID rejection，以及每條metric/count公式、集合上下界與diagnostic subset bound的獨立矛盾；fake不得被寫成production runtime或真MySQL evidence。
- [ ] 7.7 將logical tables與`REFERENCE ONLY` DDL交付external `bim-control` owner對接；驗證health以identity/digest綁publication，publication first event、每筆health與receipt均以四欄event tuple綁ledger，receipt另以identity/digest/registration ACK tuple綁publication；所有event IDs／digests採`ascii_bin`、registration/publication identities採`utf8mb4_0900_bin`。External owner須對16 KiB InnoDB page、`ROW_FORMAT=DYNAMIC`、2,952／3,072-byte最大ACK key與live DDL做migration preflight；不相容環境須fail或採不截斷logical identity的collision-safe等價physical key。本repo不得執行migration、持DB credentials、直接SQL或宣稱cloud MySQL已寫入。
