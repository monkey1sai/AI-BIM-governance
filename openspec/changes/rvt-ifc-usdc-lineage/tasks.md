...
- [ ] 7.6 擴充test-only cloud fake與contract/integration tests，模擬201 commit、200 replay、bad HMAC、ambiguous／mismatched publication identity、同event ID異raw body 409、同identity／manifest異canonical content 409、403 tenant mismatch、422 missing parent、malformed ACK、lost ACK/restart、late older health event、equal-observed-at tie-break、offset／sub-microsecond／leap-second／out-of-range observed-at rejection、health restore/tombstone、non-tombstone tombstone ID rejection與每條metric/count公式的獨立矛盾；fake不得被寫成production runtime或真MySQL evidence。
