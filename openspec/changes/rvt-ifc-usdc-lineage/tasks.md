## 1. Predecessor 與 migration gates

- [ ] 1.1 確認 active MinIO changes已merge/archive；接著強制 `align-frontend-design-system-reference` closeout → `migrate-console-to-hifi-design` rebase並撤銷／調和repo外origin與`VerifyOrigin`假設至 `docs/plans/*.html`唯一authority → `migrate` closeout → lineage rebase最新`origin/main`。不得只確認兩者closeout而把衝突authority一起archive；以 `npx openspec list` 與 strict validation 留證。
...
- [ ] 2.6 將HMAC raw-body canonicalization、±300秒skew、header/body event match、201/200 ACK、403/409/422與malformed/202 protocol failure建立contract tests；reference MySQL 8 DDL保持醒目`REFERENCE ONLY`，不得作repo migration。
...
## 3. bim-review-coordinator ownership
...
- [ ] 3.3 實作 active-result pointer、candidate results、`result.compare` read-only compare、capability-gated promote/rollback與append-only activation/transition audit；只接受 `AVAILABLE + succeeded|succeeded_with_warnings` selectable target，拒絕cross-job、failed/cancelled與non-AVAILABLE target。
...
## 4. bim-streaming-server conversion authority
...
- [ ] 4.4 產生 JSON/CSV alignment reports 與三種 ratios；denominator 0輸出 `ratio=null`／`not_evaluable`，partial alignment使用 `succeeded_with_warnings` 並保留完整 numerator/denominator/status/diff counts。
...
## 5. Runtime admission 與 Kit release
...
## 6. Callback 與 external control-plane boundary
...
- [ ] 7.3 在formal ResultManifest與四個refs完整驗證後enqueue`lineage_result_published`；failed/cancelled contract-complete formal result也發布稽核locator/summary但維持non-selectable，diagnostic/temp/invalid result排除，source READY不發布。
- [ ] 7.4 實作`POST /api/v1/lineage-publications` HMAC raw-body client與strict 200/201 exact ACK；published依identity/digest/immutable content冪等，health新event ID append、同event/body replay、同event/different body 409；另實作403/422 binding與retryable/non-retryable分類，202/empty/mismatched 2xx不得標DELIVERED。
- [ ] 7.5 實作預設5次exponential backoff＋jitter、transient dead-letter cooldown auto-reconcile、deterministic manual replay，以及`VERIFIED|MISSING|INTEGRITY_FAILED|TOMBSTONED` append-only health events；health payload不重送summary，receiver以publication identity join並驗證original result ID/ref/digest；每次accepted delivery append receipt row，missing/integrity需兩次獨立確認，restore可回VERIFIED。
- [ ] 7.6 擴充test-only cloud fake與contract/integration tests，模擬201 commit、200 replay、bad HMAC、409 digest conflict、403 tenant mismatch、422 missing parent、malformed ACK、lost ACK/restart、health restore/tombstone；fake不得被寫成production runtime或真MySQL evidence。
- [ ] 7.7 將logical tables與`REFERENCE ONLY` DDL交付external `bim-control` owner對接；本repo不得執行migration、持DB credentials、直接SQL或宣稱cloud MySQL已寫入。

## 8. docs/plans HTML authority 與 lineage console
...
## 9. Integration、migration 與 durable docs
...
- [ ] 9.5 跑`npx openspec validate rvt-ifc-usdc-lineage --strict`、JSON Schema valid/invalid fixture validation、affected service tests、GitNexus`detect_changes`與PR local preflight；未具真Kit/WebRTC/design evidence時不得宣稱full-system completion。
