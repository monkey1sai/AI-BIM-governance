# a1-lineage-crosscheck-view — A1 治理摘要卡 + #lineage 交叉比對頁

> 資料/後端規格權威 = `openspec/changes/rvt-ifc-usdc-lineage/`（三 ratios、10 fixed counts、六態 outbox、capability 授權、cloud metadata-only 邊界）。本 capability 只定義 UI/IA/誠實 provenance，MUST NOT 重定義或偏移該 change 的資料語意。本 spec 同時補位 `lineage-governance-console` 的設計 gate（docs/plans HTML approved screens，見本 change `drafts/`）。
