...
> 10.4：`git diff --check` 乾淨；coordinator `npm run verify` 綠（tsc + vitest **130**）；repo-root pytest **6**；streaming pytest **5**；`openspec validate --strict`（change valid、specs 19/0）。
