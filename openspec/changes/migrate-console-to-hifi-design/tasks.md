...
- [ ] 7.4 逐條核對 `edge-console-operator-frontend` 與 `unified-governance-console` 兩份既有 spec 的相關 Scenario 仍成立（依 design.md Risk 項，行為層面的不確定 SHALL 停下澄清，不視為理所當然通過）。2026-08-12：逐條稽核產出 `artifacts/2026-08-12-hifi-consumer-spec-scenario-audit.md`——66 個 Scenario 全數走過（edge-console-operator-frontend 30 個、unified-governance-console 36 個），58 HOLDS、5 HOLDS-WITH-NOTE、0 STALE、**3 UNVERIFIABLE**（皆需真實部署 stack／GPU／Kit runtime 才能端到端驗證的 browser E2E 場景，本 session 純程式碼稽核無法逐字確認；三項均不在本次遷移實際改動檔案範圍內，見 audit 文件「總覽」file-scope 證據）。因存在 UNVERIFIABLE 項，依規範不勾選本項；3 項清單與根因見 audit 文件「UNVERIFIABLE 項清單」一節，升級請 coordinator／使用者裁決是否另開部署驗證任務。
