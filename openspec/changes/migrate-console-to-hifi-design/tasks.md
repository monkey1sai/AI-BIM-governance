...
- [ ] 8.2 確認本 change 未修改 `unified-governance-console`、`edge-console-operator-frontend`、`align-frontend-design-system-reference` 任一既有 spec/change 檔案本體（僅新增 `console-design-token-authority`）。2026-08-14：本項失效並取消勾選——successor crosswalk 已修改 `align-frontend-design-system-reference/proposal.md` 與 `successor-crosswalk-migrate-console-to-hifi-design.md`。archive 前須重核，或把本項改寫為允許 crosswalk-only 編輯。
