...
`_materialize_unmapped_entities` previously ran an O(N) Python loop with per-entity USD-level calls. The options below were surfaced in `/opsx:explore` as **pre-measurement hypotheses** and then annotated with the §2 baseline profile measurements (see the "Baseline-annotated effect" column). The final selection is recorded in the "Selected Option(s)" subsection below; **Option 4 (sidecar carrier) + Option 3 (chunked progress writes) was selected** after the canonical baseline showed `xform_define_seconds=365.5s` (97.4% of materialization). Option 1 was rejected because even a 5× ChangeBlock improvement still projected over the 600s budget. The table remains in this document as the audit trail of options considered before measurement, not as an open decision.
...
- Per-operation breakdown within materialization:
  ...
  - `sidecar_append_seconds = 0.0s` — baseline did not exercise the sidecar path. (In the post-change run this field measures the in-memory list/dict append cost; the actual `entity_index.json` JSON dump is tracked separately as `quality_metrics.sidecar_write_seconds`.)
