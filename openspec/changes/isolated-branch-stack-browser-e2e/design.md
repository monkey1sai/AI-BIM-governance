...
- **不是 design gate**：pixel diff 與 semantic states 由 `verify-design-system-reference.ps1` / `verify-design-system-visual-result.ps1` 判定，與本 capability 正交。隔離 stack 的截圖是 functional evidence，不是 golden。
  - **該路徑的紅燈史**（2026-07-29 重跑輪更新）：`design-semantic-visual` 曾於 `13033cb` FAILURE（`workspace.a4.default` diff ratio `0.2794`／`0.3186` > `0.01`，成因＝`#a4` route IA 遷移使 golden 描繪的已填充 A4 dock 面板無路由可達），**已由 #429（`2b9573e`）就地重核 A4 golden 解除，現 main（`bfcc433`）為 success**。詳見 `proposal.md` D-1／D-15 與「三層交叉對抗驗證」節。原則不變：「design gate 由既有路徑判定」不得被讀成「可由本 change 的 evidence 推論其狀態」——無論該 gate 當下是紅是綠。
...
| `design-semantic-visual`（**advisory job，非 branch protection 的 required context**）的狀態隨 baseline 變動，PR body 若照抄舊敘述會失真 | 時間線：main `13033cb` failure → #429 起 success；本 PR 於 merge origin/main（`bfcc433`）後該 job 實跑並 **pass**。task 6.6 要求 PR body **據實描述當下狀態**（含 #429 解除事件），不得照抄任何歷史紅燈敘述 |
