...
- **#29 [L]**：2 個 untracked evidence（`docs/evidence/.../l4-artifact-2026-05-28/ifc_semantic_sidecar.json`、`docs/verification/.../dev-health-check-evidence.json`）git mv 到各自 archive sibling 的 `evidence/`（符合 `documentation-source-of-truth` spec）。**只搬 untracked evidence JSON**，同目錄 already-tracked 的輔助 `.md`/`.py`/`runbook.md`（產生/解讀 evidence 的工具，非 artifact）留原處（使用者拍板）。
...
- **Affected code**：`bim-streaming-server`（#2 comment+test）、`bim-review-coordinator`（#6 comment+test、#36 test+comment）、`services/kit-manager-api`（#26 settings+main+test）、`scripts/deploy.ps1`+`check-web-plane-docker.ps1`（#23）、`openspec/changes/archive/<2 change>/evidence/`（#29 搬移）。
