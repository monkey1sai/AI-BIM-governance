...
選擇：hybrid mode 下，coordinator container 的 `STREAMING_CONVERSION_API_BASE` / `CONVERSION_API_BASE` 必須指向 host-native conversion authority，例如 `http://host.docker.internal:49101`。
...
選擇：hybrid mode 必須文件化並驗證 OS-specific host bridge profiles，而不是把 `host.docker.internal` 當成所有 Docker engine 都可靠的固定規則。
...
| `linux-host-gateway` | Linux Docker Engine | `http://host.docker.internal:49101` with compose `extra_hosts: host.docker.internal:host-gateway` | loopback-only 不可達時，service 必須 listen on Docker bridge 可達的 host interface |
...
- Fast MVP docs 不得指示 operator 把未認證 coordinator endpoint 直接暴露到 Internet。

### Decision 3: stream config 使用 browser-visible endpoint

選擇：coordinator 回給 viewer/browser 的 Kit endpoint 必須是 browser-visible host，例如 `127.0.0.1:49100`（local single-machine）或部署者明確設定的 LAN/DNS host，而不是 `host.docker.internal` 或 container-local hostname。
...
- `49100 listening` 不代表 WebRTC media/datachannel 已完整通過，但它是 hybrid deployment readiness 的 minimum host-native runtime probe。
- `conversion succeeded` 不代表 `model.usdc` / mapping / manifest refs 對實際載入它們的 runtime 可達。
...
選擇：hybrid mode 下，轉檔完成後的 derived artifacts 必須留在 host-native conversion authority 的 artifacts root，以 `<artifacts_root>/<conversion_job_id>/` 隔離；每個 succeeded job 至少發布：
...
- cleanup 預設為 operator-driven；runbook 必須說明 artifacts root、jobs dir、如何查目前磁碟占用、如何刪除 demo 產物。實作若新增自動 retention，必須先避免刪除仍被 review session / callback outbox 引用的 artifacts。
- 文件與 logs 可以顯示路徑與 ref，但不得把大型 artifact bytes 或 secret token 輸出到 log。

## 風險 / 取捨

- **Risk: `host.docker.internal` 在不同 Docker engine 行為不同。** Mitigation：runbook 必須 document Windows Docker Desktop default path and Linux `host-gateway` override；check script 必須以明確 container-to-host diagnostic 失敗。
- **Risk: Linux container 無法連到只 bind loopback 的 host service。** Mitigation：Linux profile 必須 include `host-gateway` or explicit host address guidance，且 check output 必須說明目標機器是否需要 widen `STREAMING_CONVERSION_HOST`。
- **Risk: operator 誤把 hybrid pass 當成 Docker GPU Kit pass。** Mitigation：spec/runbook/check output 必須標示 `hybrid_web_plane_host_native_runtime`，且不得把 `runtime_image_kit_launcher` 更新為 passed。
- **Risk: browser-facing endpoint 與 container-facing endpoint 被混用。** Mitigation：compose env names and docs 必須分離 `HOST_CONVERSION_API_BASE` 與 `KIT_SIGNALING_HOST` / browser stream config。
- **Risk: artifact refs 使用只有單一 process 可解析的 localhost base。** Mitigation：runbook 必須說明 `STREAMING_CONVERSION_PUBLIC_ARTIFACTS_URL` 是 runtime-visible URL，且 check script 必須在 conversion result 存在時至少測試 `model.usdc`、mapping、manifest refs。
- **Risk: Docker web plane 啟動時 host-native services 尚未啟動。** Mitigation：start script 可允許 web plane 先啟動，但 check script 必須用 `container_to_host_conversion=blocked` 或 `host_native_kit_probe=blocked` 回報 next command。
- **Risk: local `.env` secrets leak into docs/logs。** Mitigation：templates 必須使用 `.env.*.example`；scripts 只印 variable names/status，不印 secret values。
- **Risk: 重複 demo conversion 讓 artifacts 累積並填滿磁碟。** Mitigation：runbook/check output 必須 identify `STREAMING_CONVERSION_ARTIFACTS_ROOT` and document manual cleanup / retention expectations；自動刪除不在本 change 範圍，除非能證明沒有 active session/outbox reference。
- **Risk: `0.0.0.0:8004` 被誤認為 production public exposure。** Mitigation：docs 必須標示它只屬 LAN/single-machine exposure，public/cross-company exposure defer 到 separate edge-security design。

## 遷移計畫

1. 新增 hybrid compose override 與含 non-secret defaults 的 `.env` example。
2. 新增 Docker web plane hybrid mode 的 start/check helper scripts。
3. 更新 fast MVP runbook，把這條路徑納入 fast MVP 後可部署的單機標準流程。
4. 在本機驗證 compose config 與 health checks。
5. 保留既有 host-only `scripts/start-all.ps1` 與 Docker GPU profile。
