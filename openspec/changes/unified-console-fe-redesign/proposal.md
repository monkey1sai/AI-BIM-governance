...
- Non-goals：不在此 change 強制 streaming-server（host GPU Kit）DataChannel 的 `source_client_id` 後端權威（需 host GPU runtime 真驗，列已知限制）；不做 CH-A 完整設計系統 token；不接 spectator 多人協作；不改 governance-service / streaming data shape。
