...
`element_mapping.json` 第一版即支援 one IFC GUID 對多個 USD prim path。每筆 mapping 應提供 `ifc_guid`、`primary_usd_prim_path`、`usd_prim_paths`、`mapping_method`、`mapping_confidence` 與可選 diagnostics。`primary_usd_prim_path` 是 UI / highlight / focus 的主路徑，讓現有 viewer 與 DataChannel command 可以維持單一 focus target；`usd_prim_paths` 保留完整映射，供後續 multi-part highlight、debug 與 coverage 分析使用。
...
- [Mapping correctness risk] IFC GUID 可能在轉檔後遺失或被拆成多個 prim。緩解：mapping schema 第一版即支援 `primary_usd_prim_path` + `usd_prim_paths`，並保留 confidence、unmapped reason 與 diagnostics。
...
- `primary_usd_prim_path` 作為 UI / highlight / focus 的主路徑，`usd_prim_paths` 保留完整映射。
