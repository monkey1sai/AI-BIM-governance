...
Trusted merge event必須來自允許的repository、`main` base、closed且merged PR、observed merge commit與fresh convergence SHA相等；任何自述布林、wrong repo、stale SHA或partial collector均fail closed。Artifact、target與attestation schemas皆closed，未知欄位拒絕；artifact provenance至少綁source commit、immutable content digest、builder與policy digest，deployment target只揭露non-secret target ID、kind、role、fingerprint與opaque lease ID。

Single-flight key使用 `environment + service`，相同delivery與artifact digest可idempotent replay，不同delivery或digest不得共用active lock。每個attempt append-only保存request、artifact、target、state sequence、retry lineage與terminal attestation；同一exact input只允許bounded transient retry。Canary、promotion與post-deploy驗證只能消費同一artifact digest；promotion mismatch不可進 `ACTIVATED`。Rollback只准使用attempt開始前已pinned的known-good immutable artifact，成功readback與health證明後才是 `ROLLED_BACK`；任何rollback evidence不完整即 `HELD`。

Candidate workflow與repo-local controller沒有live deployment credential，也不得讀取private inventory。它們只驗證event／contract、執行negative tests並產生sanitized `PROVISIONING_REQUIRED → HELD` attestation；只有owner完成external artifact store、trusted runner、credential broker、protected GitHub Environment與canonical target live attestation後，外部executor才可執行上述state machine。這個repo evidence不得冒充installed或live activation。
