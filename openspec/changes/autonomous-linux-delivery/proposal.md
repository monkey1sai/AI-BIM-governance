...
- CI／review findings 新增 closed disposition layer：每個 finding 都必須以 exact-head evidence 裁決為 `FIX`、`REJECT`、`ACCEPT_RISK` 或 `DEFER`；`Resolve conversation` 表示裁決已完成，不表示一律修改 code。Confirmed in-scope blocker 必須 `FIX`，`REJECT` 必須有反證，`ACCEPT_RISK` 只適用 policy 明定的 non-blocking risk，`DEFER` 必須 out-of-scope 且綁定 follow-up Issue。Source-pinned machine gate 只能在 disposition convergence 與 zero unresolved threads 之後發布 success。
