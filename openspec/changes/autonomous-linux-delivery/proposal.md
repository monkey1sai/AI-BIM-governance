...
>
> **Status: active specification; live autonomous authority remains `HELD` until the canonical Parallel Delivery Fabric activation record is independently validated.**
>
...
- **Phase 0 live rule**：`main` 仍要求既有 human／CODEOWNER counted approving review。只有 canonical Parallel Delivery Fabric activation record 以 exact base、policy digest、source-pinned external CheckRun 與 `AUTONOMOUS_ACTIVE` phase 驗證後，才可另行啟動移除舊 review gate 的外部設定流程。
...
- 禁止 candidate branch 修改或執行自己的 merge adjudicator；self-referential mechanism change 由 immutable external checker與獨立 machine authorization lane裁決。historical lifecycle ledger維持 byte-frozen；future fixpoint或reconciliation closure由單一 ordinary protected PR處理。trust root unavailable 時結果只能是 `HELD`，不得退回較弱自我驗證。
...
- 一次性 bootstrap 必須先建立並驗證 external trust root、required check source binding、短效 credential與canonical Linux deployment runner。此 change 的 promotion mode 僅可為 mutually exclusive `single_pr|direct_stack`；mode一經綁定不得切換。external terminal class SHALL只允許 `DELIVERED`, `FAILED` or `HELD`，`STACK_*`只屬內部 group saga detail，永不得成為外部terminal class。未完成正負 attestation與canonical activation record前不得宣稱 autonomous delivery已啟用，也不得把 branch protection 的 required approving review count 調為0。
...
- **Breaking governance boundary**：移除 required human review 是未來、record-gated policy change；在 Phase 0與任何未驗證 activation phase，approval bot／固定 User service account仍是目前 counted review path，external machine path只可shadow或`HELD`。
...
- **Self-referential boundary**：此 change 本身會修改 merge／verification／deploy mechanism surface，但 historical lifecycle ledger必須 byte-frozen；future fixpoint/reconciliation wording均由 canonical Fabric change取代為普通 protected PR closure。本 proposal 不構成 live設定、merge或部署授權。
