...
- [ ] 5.6 將viewer origin完整失敗態矩陣接到既有i18n keys；本切片已收斂runtime-command-rejection diagnostics、changed-unconfirmed binding、rejected stage-load，以及2026-08-12新收斂的stage-load-timeout（見下方RED/GREEN證據：兩觸發路徑皆有可見overlay/雙語診斷文案/late-result不覆寫證明，並新增`data-stage-failure-reason`狀態專屬test anchor）。2026-08-17新增收斂no-session、viewer-origin-missing（含refresh動作）、lease-occupied（驗證＋holder-privacy負向斷言）三態（見下方同日期證據）。2026-08-17再收斂session-preparing（runtime/status status=created，與not_listed分離的專屬診斷、專屬test anchor與refresh動作。**刻意不封鎖attach**：既有跨頁契約（pages.tsx KG/Review跨頁連結與CoordinatorPage rt-crosslinks，比照後端isSessionMutable）已裁定「created尚未綁Kit但已可attach」，封鎖會反向違反同一條誠實鐵律，故本態只增加可見診斷，並以正向斷言鎖住manual-start仍可用）、lease-expired（coordinator對heartbeat的權威終局拒絕404 `Viewer lease not found or token invalid.` / 409 `Review session is not active.`才升級；升級時卸載viewer並清除first frame/DataChannel/stage證據，附reclaim動作；503等不確定拒絕維持靜默重試並有負向斷言）、first-frame-timeout（持有active primary lease但在有界預算內未觀察到第一幀時轉為可見失敗態，retry先release再以新viewer身分重新claim以強制乾淨remount）共3態，各有雙語文案與逐態test anchor（`*-session-preparing`／`*-lease-expired`／`*-first-frame-timeout`）。誠實邊界：first-frame預算取45秒（與viewer端`STAGE_LOAD_TIMEOUT_MS`同級），**不是**量測得出的TTFF SLO —— 權威TTFF上限由`gpu-session-baseline-and-idle-reclaim` task 1.4的本地實測基準持有，尚未產出；逾時文案只主張「本瀏覽器在預算內未觀察到第一幀」，不替Kit／GPU端斷因。仍未收斂：stream-disconnected（`EmbeddedViewer`未對外暴露任何連線中斷／WebRTC狀態回呼，要誠實呈現需先擴充viewer postMessage橋契約，屬另一切片）、gpu-unavailable（coordinator claim／heartbeat路徑目前沒有可判定的GPU不可用錯誤碼，在後端給出權威碼之前任何前端呈現都會是捏造成因）。因此checkbox維持open、production/full completion維持no
