...
- [ ] 1.3 對 `A4SemanticSearchPage`、`governanceClient`、`engine.search`、`proofs` 執行 `gitnexus impact -d upstream -r AI-BIM-governance`，HIGH／CRITICAL 須在 PR body 揭露補強策略；index stale 時先 `node .gitnexus/run.cjs analyze --index-only` 或依 `docs/agents/gitnexus-usage.md` 走 unavailable gate。
