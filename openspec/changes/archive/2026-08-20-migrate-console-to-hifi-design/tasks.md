...
- [x] 6.4 **human owner only**：同步 origin（`C:\Repos\design\desigin-system`）與受保護的 repo `docs/plans/` 正本；AI 只提供 parallel proposal/draft，不原地寫入任一正本。**Status: deferred-this-change 2026-08-20**（terminal for archive：owner 2026-08-20「繼續執行直到 archived」。本 change 不執行 origin↔repo 正本對齊；下次 human 改 Hi-Fi 正本時再做 R-A4。不另開 successor change。）
