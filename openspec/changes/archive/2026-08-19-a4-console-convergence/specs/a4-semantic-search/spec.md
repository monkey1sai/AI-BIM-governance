# a4-semantic-search（本 change 的 delta）

本 delta 只涵蓋「把分岔的 A4 前後端收斂為單一 canonical 實作」。Requirement 名稱刻意沿用 deferred 母版 `a4-semantic-search-model-qa`，以利日後 thaw crosswalk；母版其餘 Requirement（session auth principal／lease、3D handoff 完整語意、Ornith live evidence、semantic completion）**不在本 delta**，仍由母版持有，不得平行實作。
