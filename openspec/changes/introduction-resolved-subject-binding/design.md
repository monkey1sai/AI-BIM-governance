...
- **CI 具現方式（挑戰 B3 證實）**：`agent-governance.yml` 用 plain `actions/checkout`（PR merge ref）執行 **HEAD 腳本**；唯一 base-pinned 具現是 `pr-review-agent.yml`。machine-truth comparator（verify CLI）不在任何 `.github/` workflow 中被呼叫；CI 只以 node --test 跑其 fixtures。
