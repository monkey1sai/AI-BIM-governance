...
| 15 | sec1／`c1-badges` | `c1-badges`（鐵律 3 badge） | normative(doc) | `grep -c "byte-for-byte" $MAIN` → 0；`grep -c "known gap(不是 pass)" $MAIN` → 1 | Wave 2 | 裁1 | `eaeff4b` |
...
| 20 | sec4／`c4-contract-conversion-result-callback` | `c4-contract-conversion-result-callback` | normative(doc) | `grep -c "follow-up metadata-allowlist(planned)" $MAIN` → 1 | Wave 2 | 裁1 | `1e5daea` |
