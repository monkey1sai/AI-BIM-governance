...
| 9 | KIT_SPECTATOR_COUNT 預設 0；邀請連結真複製；內嵌 spectator streamRole | ADDED R-B6（同裁3，「spectator 與 issues 裁決落地方向」scenario 前半） | item18=`c7-ch-schedule-table`（CH-I 列內「spectator 政策(KIT_SPECTATOR_COUNT 預設 0、邀請連結真複製)」字樣，$MAIN sec7） | `0cb7989`（同裁3） | `openspec validate --strict` 綠；本次親跑 `grep -c "KIT_SPECTATOR_COUNT 預設 0、邀請連結真複製" $MAIN` → 1 |
...
| 3/9（item18） | `0cb7989`（單一 commit：§07 補 CH-H semantic viewer 家族＋CH-I 內嵌 viewport 新期，翻轉 §03 handoff badge） |
| 10（item19） | `6729fea`（單一 commit：§03 補雙軌現況誠實入文徽章） |

## 3. 方法論與交叉核對備註

- **「裁決→data-canon-id」對映規則**：design.md §2「落地位置」欄逐字明列 R-C2 item # 者（裁1／裁3／裁6／裁7／裁8／裁9／裁10），本檔直接取該 item 於 `crosswalk.md` 已驗證的 data-canon-id／commit；design.md §2 未列 item # 者（裁2／裁4／裁5／裁11），本檔如實標「無」或「無專屬」，另附 design.md §3「24×11 追溯矩陣」（task 5.5／`6c12e8d` 核對版，零缺格）之矩陣覆蓋關係供交叉參考，**不臆造** design.md §2 未載明的 item 歸屬。
