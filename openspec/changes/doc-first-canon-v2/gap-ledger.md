...
| 9 | canon-defect | immediate-gap | MAIN:653（§08 R3「每個數據面板掛 `ProvenanceTag(mock \| live)`」，非實碼詞彙）；MAIN:692（共用元件清單沿用同一舊詞彙 badge）；README:38（四條鐵律 R3 子句同一 `ProvenanceTag(mock\|live)` 殘留）；code：`web-viewer-sample/src/console/data.ts:6`（實碼 `Prov` type 7 值：asbuilt/artifact/demo/p1/p15/p3/p4，元件 `ProvTag`） | 2026-07-17 | P2 | `c8-r3-provenance`（＋巢狀 `c8-r3-oq1-open-decision`）；README `<!-- canon:r-authority-order -->` 鄰近段落（R3 子句無獨立 canon 錨，隨 §3 條列一併植錨） | — |
