...
**裁決不對映失真而以條文化落實**：裁 4（需求正本邊界→R-B4）、裁 5（變更控制→design-canon-change-control）、裁 7（撈回→R-C4）、裁 11（reconcile→follow-up）不直接改任一失真項文字，而以獨立條文/follow-up 承載。矩陣證每項失真至少對到裁 1（doc-first 為總綱），且每條裁決或覆蓋失真或明列條文化落實 → 零缺格。裁決 1 掛「1 MODIFIED（權威序 body）＋1 ADDED（R-B1 偏離處置序）」。
