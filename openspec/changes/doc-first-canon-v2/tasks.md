...
- [x] 3.1 **item 22（核心翻轉）**：§08 權威順序表改寫為 doc-first（現寫 code＋tests 第 1、書面需求第 4 → 翻為 docs/plans 需求第 1、code＋tests 為 runtime 現況查證面）。**DoD/draft-submitted**：§08 該區塊 draft diff＋Crosswalk 對號。(2026-07-18 PASS:draft `c8-authority-table`(draft:611-617)七列全換為 doc-first 序——1 使用者最新明確指令/2 docs/plans 需求正本(§01–§08;前端視覺/互動面=Hi-Fi+ai-bim-governance.css 最高)——依領域分工/3 tests/contracts/*.json(payload 委任,§04 保留)/4 AGENTS.md/OpenSpec(治理程序)/5 Prompt Board 文字(僅意圖參考)/6 應用場景圖(僅視覺上下文)/7 圖中示例數字(fixture);表頭「寫進 AGENTS.md」改參照語意(draft:609)、§08 尾 badge 同步改參照語意(draft:730);新增誠實半句「code+tests=runtime 現況的查證面;code 偏離本正本=implementation gap 待修,不得以文件宣稱 runtime 已完成」(draft:619);carve-out 五項迴歸複驗全 PASS(未觸碰任一 carve-out,§04 payload 委任/鐵律1-2/README§3.4/README§3.5 誠實子句/§07:575 diff 皆空或子句仍在);Crosswalk 對號＝design.md §4 既有列「22｜§08 權威順序表｜normative(doc) 翻轉｜Wave1｜裁1,2」,本次改寫手法與之一致;`npx openspec validate doc-first-canon-v2 --strict` 綠)。
