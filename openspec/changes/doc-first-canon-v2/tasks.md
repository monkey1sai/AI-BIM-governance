...
- [ ] 2.1 撰寫 MODIFIED「Workflow v3…」body：翻為 doc-first 權威序、內部裁決序、保留誠實鐵律半句＋非矛盾 scenario。**DoD/draft-submitted**：delta 完成＋header 逐字重現＋validate 通過。
- [ ] 2.2 撰寫 MODIFIED「文件分工調整必須走 PR 治理流程」body：加手寫正本面 cross-ref design-canon-change-control R-A1。**DoD/draft-submitted**：delta 完成＋header 逐字重現。
- [ ] 2.3 撰寫 ADDED R-B1（偏離處置序＋三態分類 code-defect/canon-defect/undecided）、R-B2（誠實鐵律非矛盾）、R-B3（carve-out 清單＋diff 斷言）、R-B4（需求正本邊界）、R-B5（R2 三態＋ProvTag 詞彙＋planned 三態 class）、R-B6（內嵌 viewport 防護＋spectator/issues 裁決）。**DoD/draft-submitted**：各 ADDED requirement ≥1 scenario＋validate 通過。
