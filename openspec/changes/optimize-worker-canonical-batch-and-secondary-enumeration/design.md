...
## Post-implementation canonical evidence (2026-05-15, v3)

Full 13-file canonical batch executed three times during apply:

- **v1** (`--limit 13 --timeout-seconds 600 --profile-source-entities`, monolithic single-process): 13/13 converted, but every fixture `unmapped_count=2`, `coverage_status=unlocked` → `outcome_distribution.passed=0`, `passed_with_quality_warning=13`, `minimum_coverage_locked=false`. Disproved the pre-change hypothesis that the residue was no-GUID geometry-shape entities.
- **v2** (after the no-GUID-only key fix): fixtures that completed still showed `unmapped_count=2`; 4 fixtures failed with Windows `PermissionError [WinError 5]` on `<jobs_dir>/conv_*.phase.json.tmp` rename (high-frequency progress writes inside the long-lived process, jobs_dir under the git worktree → AV/indexer contention). Diagnosed the real root cause via `ifc_index.json` summary: `guid_count=73,743` vs `73,745` entities carrying a GlobalId → exactly **2 duplicate-GlobalId instances** in the source model.
- **v3** (after `ifc_entity_key` made unconditionally unique via the 1-based enumeration index; `WORKER_JOBS_DIR` / `WORKER_OBJECTS_ROOT` relocated outside the worktree): **all 13 fixtures `status=passed`, `coverage_status=pass`, `unmapped_count=0`, `mapped_count == source_ifc_entity_count == 1,604,773`, `minimum_coverage_baseline_locked=true`**; `outcome_distribution.passed=13` (all other buckets 0); batch `status=passed`, `minimum_coverage_locked=true`. Secondary profile: `guid_extraction≈10.63s` + `name_extraction≈10.03s` ≈ 20.66s of a `source_entity_enumeration≈27.4s` against `conversion_total≈230.6s`.

**This change's stated goal is achieved.** The duplicate-GlobalId fix (Decision 1 family — `ifc_guid` keeps the real, possibly-duplicate GlobalId; no synthetic identifier; all-entity denominator unchanged) closes `unmapped_count` `2 → 0` and the canonical batch locks the baseline. Detailed evidence is captured in `docs/verification/2026-05-14-worker-canonical-batch-and-secondary-enumeration.md`.

## Decisions (post-explore, 2026-05-15)

9. **Secondary `guid_extraction` / `name_extraction` optimization is deferred (Option C1 deferral arm).**
   - Measured cost: `guid_extraction≈10.63s` + `name_extraction≈10.03s` ≈ 20.66s aggregate. This is a cost figure, not an achievable saving; realising ≥5s of it requires IfcOpenShell attribute-access optimization with real `ifc_guid` / `name` fidelity risk, which the predecessor explicitly deferred.
   - Decision: record the measurement, do **not** land an optimization in this change, emit `optimization_applied=deferred_to_follow_up` with follow-up candidate `optimize-worker-secondary-enumeration`. This is exactly the deferral path Option C1 permits.

10. **Queue-based single-fixture batch orchestration and post-USDC artifact file-management are OUT of scope for this change (route α).**
    - Context: the monolithic `--limit 13` single-process run is fragile (all-or-nothing; v2 lost progress to a Windows file-lock at fixture 4) and produces ~1.58 GB of derived artifacts per fixture (`element_mapping.json` 663 MB + `ifc_index.json` 578 MB + `entity_index.json` 330 MB vs `model.usdc` 9.4 MB — the only runtime artifact). One canonical session accumulated ~58 GB across v1/v2/v3, much of it inside the git worktree.
    - Decision: this change keeps its original single-pass, no-scheduler scope (Non-Goals + Decision 7 stand). v3 already proved the coverage goal with the monolithic path, so this change closes out on that evidence. Queue orchestration + file-management become a **clean successor change**, not a scope expansion of an already-proven, in-review change. Rationale: folding three concerns (coverage fix / queue / retention) into one near-done PR contaminates a finished deliverable, contradicts this change's own Non-Goal, and would invalidate the already-collected v3 evidence (orchestration would change).

## Successor handoff (settled during 2026-05-15 explore; created only after this change is archived)

Per **NoSuccessorWhilePredecessorOpen**, the successor change is **not** created yet (this change is still open: PR in review, not archived). These design decisions are recorded here so the successor proposal starts from settled ground; the successor's formal `change-id` is resolved via `/change-id-resolve` only after this change's archive PR merges.

- **Provisional successor name:** `queue-batch-dispatch-and-post-usdc-artifact-retention`.
- **Manifest-as-index:** a single persisted `batch_queue.json` is simultaneously the work queue *and* the artifact index — one row per canonical `source_id` (the existing `dev_sources` sha256 identity) carrying `status`, `conversion_job_id`, recorded artifact paths, and a retention class. `outcome_distribution` / `minimum_coverage_locked` semantics are unchanged — only their data source moves from an in-memory list to the persisted manifest.
- **Retention strategy A (chosen):** after coverage is computed, the giant 1.6M-row JSON arrays (`ifc_index.json`, `element_mapping.json`, `entity_index.json`) are dropped; only `model.usdc`, `metadata.json`, `usd_index.json`, the coverage **summary**, and the small `unmapped_*` lists are retained. Target footprint ≈130 MB-class instead of ~58 GB. Downstream Handoff Framework already confirms coordinator/viewer/streaming consume only `model.usdc` + renderable-prim mapping (~7k rows), never the 1.5M-row sidecar arrays, so dropping the full arrays post-coverage is semantically safe. Open sub-question for the successor: whether to stop persisting the full arrays at all and stream-compute coverage (touches the converter; larger blast radius).
- **Failure semantics (preserve Decision 7 spirit):** `failed` / `timed_out` queue entries are **not** auto-retried; resume only re-runs entries that never received a recorded outcome (crash mid-flight). Re-running a recorded failure requires an explicit `--retry <source_id>` (the durable-state equivalent of today's "run `--limit 1` in a separate session").
- **Queue driver:** prefer repeated `--run-next` invocations (each a short-lived process picking the next `pending`) over a long-lived `--drain`, because short process lifetime is what root-mitigates the Windows `.tmp`-rename lock; the existing `--limit` / `--timeout-seconds` / `--profile-source-entities` one-shot CLI stays for CI.
- **Default artifact + manifest location outside the git worktree:** root-fixes both the Windows file-lock and the "36 GB inside the worktree" problem; the successor should also define cleanup of canonical-verification scratch (the `tenant_batch_verification` outputs are throwaway evidence, not durable review data).
