...
- [x] 2. intake payload 帶 `model_category`/`project_display_name`；`ifcReadyPayloadSchema` + `ExternalIfcReadyEvent` 加兩個 optional 欄位（`.passthrough()` 相容、只隨 payload 傳遞、不入 store）。
...
- [ ] 5. P7 部署區 browser E2E：對真 `bim-control` 丟多層新 key 觀察觸發 + `#conv` UI 完整顯示原始結構 + 截圖落 `docs/evidence/minio-watch-auto-intake/`（狀態：待部署區重建本 branch + 丟檔配合）。
