...
- **WHEN** watcher 已完成 baseline 且 bucket 出現新的、去 prefix/suffix 後 ≥3 段的 `{專案}/…(動態中間層)…/{種類}/{版本}/model.ifc`
