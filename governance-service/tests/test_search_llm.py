...
def test_auto_attempts_model_for_chinese_proximity_and_never_scans(a4_ifc, monkeypatch):
    import search.engine as engine_mod
    from search.llm_client import LlmConfig, LlmError

    calls = {"llm": 0}

    def unavailable_llm(**_kwargs):
        calls["llm"] += 1
        raise LlmError("llm_network_error", "test-only unavailable")

    monkeypatch.setattr(engine_mod, "chat_completion", unavailable_llm)
    monkeypatch.setattr(
        engine_mod,
        "open_model",
        lambda _path: (_ for _ in ()).throw(AssertionError("unsupported proximity must not scan IFC")),
    )
    monkeypatch.setattr(
        engine_mod,
        "load_llm_config",
...
def test_llm_bounded_reader_rejects_eof_that_arrives_after_total_deadline(monkeypatch):
    import search.llm_client as llm_mod

    clock = [100.0]

    class SlowEofResponse:
        headers = {}
    ...
    monkeypatch.setattr(llm_mod, "monotonic", lambda: clock[0])

    with pytest.raises(LlmError) as exc:
        llm_mod._read_bounded_response(SlowEofResponse(), deadline=100.010)

    assert exc.value.code == "llm_timeout"


def test_llm_bounded_reader_shrinks_socket_timeout_for_slow_drip(monkeypatch):
    import search.llm_client as llm_mod
    ...
    with pytest.raises(LlmError) as exc:
        llm_mod._read_bounded_response(response, deadline=200.010)

    assert exc.value.code == "llm_timeout"
    assert response.reads == 2
    assert response.socket.timeouts == pytest.approx([0.010, 0.004])
