...
A1「BIM 治理與模型檢核」rule-run authority。落地端內部 Python/FastAPI 服務（`127.0.0.1:49102` loopback），對真實 IFC 跑宣告式規則集，產出 governance score + failed elements + Excel。純 CPU host-native ifcopenshell，無 GPU / Kit。
...
- issue 生命週期資料庫、BCF server、IDS-XML 匯入（皆 p1/p15 後續）
