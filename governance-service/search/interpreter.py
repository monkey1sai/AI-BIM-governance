...
PROXIMITY_RE = re.compile(r"(?:\b(?:within|near|distance|proximity)\b|附近|距離|邻近|鄰近)", re.IGNORECASE)
...
def interpret_query(query: str) -> InterpretedFilters:
    ...
    if residual:
        if re.search(r"[A-Za-z\u4e00-\u9fff]", residual) and len(residual) <= 24 and not any(ch.isdigit() for ch in residual):
