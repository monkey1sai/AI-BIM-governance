...
def _issue_row_proof(
    ...
    if not isinstance(accepted_prim, str):
        accepted_prim = None
    # Preserve the canonical Issue provenance fields while also carrying the
    # exact aliases consumed by the already-landed #365 handoff verifier.  All
    # values come from the same coordinator-owned trusted context; the browser
    # cannot supply or override this snapshot.
    proof_binding = {
        **binding,
