...
- 計算域與網格配置（設定可調 B 階段，`docs/plans/building-energy-cfd-b-engine-params.md`）：`make-case`／`batch`／`converge`／`aij-case-c` 都接受下列旗標，預設值全部等於現行行為（取自 `CaseParams`，golden test `tests/test_golden_case.py` 逐位元組釘住預設產生的 case 檔）：
  ...
  - `--ground-band-height-h`：上游沿地面的低層加細帶，帶內至少要有 2 層該等級的格子，否則寫 case 時就失敗。
