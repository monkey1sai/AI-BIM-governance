...
- 細化盒（`refinement_box_mode`）：**預設 `isotropic`**（S5c，owner 2026-09-22 裁決）——以建物平面頂點重心為圓心、最遠頂點為半徑的圓（＋同樣的 1H／2H 邊距）；重心與頂點距離對繞 Z 旋轉不變，任何風向細化盒大小相同（背景 blockMesh 仍跟隨旋轉 bbox 的 COST 732 計算域，背景格數照舊隨方向變動）。代價：對長形建物，等向盒面積可達 bbox 盒的 3.5–6.5 倍（S3.1 參考建物 6 m 同層真跑格數 574k vs 511k，+12%）。`converge`／`batch --refinement-box bbox` 可回到 P1 的「隨旋轉 bbox 變動」行為做對照；`aij-case-c` 預設**維持 bbox**（單一風向、與 S5b-2 基準網格可比），要改用等向盒需顯式 `--refinement-box isotropic`。
- `aij-case-c`（S5b-2）：AIJ UWE Case C 方塊基準（Zenodo 10.5281/zenodo.15401792，CC BY 4.0；`RS_caseC.csv`／`AF_caseC.csv` 放本機 `--data-dir`，不入版控）。程式自建 3×3 個 0.2 m 立方體（中心距 0.4 m，中央 0D／1D／2D），依 `--scale`（預設 75，讓 1.5 m 行人面＝0.1D）放大；入流以 AF 剖面做 log-law 擬合取 `z0`、`uref` 取 z=D；風向 270°（吹向 +x）對應 AIJ WD 0；在量測點以 IDW 取行人面 |U|，兩邊各以「量測高度的來流風速」正規化後算 hit rate（門檻 66% 出自 COST 732／VDI 3783-9，AIJ 交叉比較採用；AIJ 指南本身未訂門檻）、FAC2、R、FB、NMSE，輸出 `aij_case_c_comparison.json`＋散點 SVG。只支援 WD 0；22.5／45 需旋轉方塊陣列而非入流。
- `aij-case-c` 的 S6 前置旋鈕（預設全部＝S5b-2 基準，一次只改一項）：`--inlet-turbulence fixed --intensity-from-af`（入口 k／ω 改為由風洞 u_rms／U（z=D）算的均勻值，取代 atmBoundaryLayerInletK／Omega）；`--wall-z0 <m>`（只換地面 `atmNutkWallFunction` 的 z0，入流 ABL z0 不動——兩者原本共用一個 `z0_m`）；`--scale 1`（模型尺度，行人面自動跟到 0.1D＝0.02 m，Re 與風洞一致）；`--cell <m>`（背景格）。`--experiment <label>` 會寫進比對文件；比對文件的 `limitations` 依旋鈕動態生成。scale 1 時流線種子的 1 m 常數偏移不隨尺度縮放，流線視覺化不具代表性（不影響 hit rate）。對應的 `CaseParams` 欄位：`inlet_turbulence`、`wall_z0_m`、`pedestrian_height_m`（service 不暴露，預設行為不變）。
...
- solver／網格預設值只定義一次：`CaseParams`（`end_time` 600、`n_procs` 8、`uref` 5、`zref` 10、`z0` 0.5、細化層級 2／1）與 `PreprocessProfile`（`sealing_leak_fraction_limit` 0.15）；`make-case`／`batch`／`converge`／`aij-case-c` 的旗標預設值與 service 的 standard preset、request schema 預設都從那裡取值（測試釘住）。`run-case` 在 `case_meta.json` 缺 `end_time` 時也用 `CaseParams.end_time`。
