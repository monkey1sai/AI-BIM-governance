...
- Hold one environment-plus-service controller lease from merge preparation through terminal attestation. A repeated identical active request may be recognized as already owned but must stop before deployment; another controller or a consumed replay key is `HELD`. The controller consumes retry evidence and appends every state transition to the previous ledger digest.
