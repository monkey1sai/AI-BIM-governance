...
Owner ruling 2026-09-03: for this skill's explicit per-PR invocations only, the counted vote may be submitted programmatically as the fixed User through the owner-approved protected broker, superseding the general "agents must not submit reviews" line in `.claude/workflows/ship-item.md`. The broker still never merges, never enables auto-merge, never resolves threads, never dismisses reviews, never pushes, and never changes repository settings. `BLIP_GITHUB_TOKEN` is vote-only and may be read only by the protected broker from the fixed `C:\Users\IOT\.grok\github-bot\.env.blip` path after owner/SID/non-reparse/protected-ACL validation; agents never read or relay its value.
...
- The vote credential is `BLIP_GITHUB_TOKEN` and may be consumed only by the owner-approved protected ProgramData broker at `C:\ProgramData\AI-BIM-governance\blip-approve\v1\run_blip_live_approve_once.ps1`. The broker validates its immutable manifest/runtime/ACL trust chain, opens only fixed `C:\Users\IOT\.grok\github-bot\.env.blip` with an exclusive handle, requires exactly one supported assignment and an owner-only protected ACL with explicit sandbox denial, and verifies the fixed reviewer login/id/type with exactly `write` permission. Direct use of the editable user-profile helper, any other `.env*`, ambient `gh` credentials, command-line/stdin/clipboard token transfer, or a direct API fallback is prohibited.
...
Validate the attestation through the protected trust chain and re-read the tuple. After explicit owner authorization for the named tuple, invoke the User broker from fixed PowerShell. The broker alone reads the fixed owner-only `.env.blip`; the agent must not read or transfer the token value.
