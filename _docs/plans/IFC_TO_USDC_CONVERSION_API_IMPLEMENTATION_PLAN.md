...
```txt
[ ] 1. 讀取 root AGENTS.md，確認 repo 邊界
[ ] 2. 檢查 git status，建立 feature branch
[ ] 3. 檢查 bim-streaming-server 既有 converter script
[ ] 4. 檢查 premake5.lua 是否註冊 converter app
[ ] 5. 建立 _conversion-service FastAPI skeleton
[ ] 6. 建立 conversion job model / job store
[ ] 7. 建立 converter_runner.py，包裝 convert-ifc-to-usdc.ps1
[ ] 8. 建立 ifc_indexer.py
[ ] 9. 建立 bim-streaming-server/scripts/inspect-usd-stage.py
[ ] 10. 建立 usd_indexer.py，透過 Kit 執行 inspect script
[ ] 11. 建立 mapping_builder.py
[ ] 12. 建立 result publishing 到 _s3_storage
[ ] 13. 建立 optional 回寫 _bim-control 的 client
[ ] 14. 新增 unit tests
[ ] 15. 新增 smoke_conversion.ps1
[ ] 16. 執行 pytest
[ ] 17. 執行實際 conversion smoke test
[ ] 18. 啟動 streaming server + web viewer 做 manual openStageRequest 驗證
[ ] 19. 更新 README / docs
[ ] 20. git diff 檢查
[ ] 21. commit
```
...
```txt
[ ] GET http://localhost:8003/health 回 ok
[ ] POST /api/conversions 可建立 job
[ ] conversion job 可完成
[ ] job log 可查看
[ ] model.usdc 產出
[ ] ifc_index.json 產出
[ ] usd_index.json 產出
[ ] element_mapping.json 產出
[ ] mapping summary 正確
[ ] _bim-control 可取得 usdc_url / mapping_url
[ ] web-viewer-sample 可使用 usdc_url 發 openStageRequest
[ ] bim-streaming-server 可載入該 USDC
[ ] git status 乾淨或只剩使用者明確未提交檔案
```
