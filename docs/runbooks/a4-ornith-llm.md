...
```powershell
Copy-Item .env.example .env         # 只在尚無 .env 時；由 operator 以 out-of-repo 管道填入值
...
- proof registry 為 process-local short-lived state；服務重啟或多 worker 沒有 sticky/shared state 時，未消費 proof 必須 fail closed 並要求重新查詢。
