...
3. 同門檻再按一次：回 **200**，`created_count` 0，兩向 `idempotent_replay: true`、issue id 不變。→ `02-findings-replay.png`
4. 讀回：`GET /api/cfd/runs/{run}` 的 `ledger.findings` 兩筆（annotation、medium、`validation_level: screening`、`opened_by: coordinator-browser`）；coordinator proxy `GET /api/governance/issues?kind=annotation` 由 0 筆變 2 筆，兩筆 `usd_prim_path` 為本 run 的行人面 prim、`ifc_guid` 空。
...
截圖裁掉右側審查面板並遮罩模型選擇器；`model_version_id` 以 `<model_version_id>` 取代；Issues／BCF 頁其他 142 筆 rule-run issue 帶真實 IFC GlobalId，未入檔也未截圖。
