...
Amended on 2026-09-23 while implementing tracer bullet 1 (§4); §2, §3 and §4 are read with this amendment. The module is `web-viewer-sample/src/coordinatorClient/` (`errors.ts`, `transport.ts`, `routes.ts`, `client.ts`, `types.ts`, `index.ts`). `routes.ts` holds `COORDINATOR_ROUTES`, one entry per calling member with its method, contract path template and tag. The transport takes that route spec with the call's template parameters and query (`request(route, { params, query, body, headers, keepalive, signal })`) and builds the path from the template itself, so no member can reach a path other than the one its route declares; every call still sends the request shape its old primitive sent. `attempt` arrives with the CFD family in bullet 2, its first user. `coordinatorClient.contract.test.ts` checks every `operationId` tag against the contract, that no `proxied` or `non_contract` route is a contract operation, and that every calling member has a route; `client.test.ts` pins each member's method and URL. `CoordinatorHttpError` also carries the parsed reply as `body`, which is how `createReviewSession` recognises the `queued_for_instance` conflict for every caller. An `error_code` that is not a contract code (lowercase snake case, at most 64 characters, the viewer's rule) is null; the viewer put `http_<status>` in its place, and that fallback moved into Window's A4 handoff detail. A 2xx reply that is not JSON is a 502 `coordinator_response_malformed`; a body read cut short by an abort, timeout or network failure rejects as `fetch` does. The viewer's `closeReviewSession` had no production caller and demanded `status: "closed"`, stricter than the contract, so it is dropped and `sessionClose` serves the route. Window calls `streamConfig` (which encodes the session id) instead of `getStreamConfig` and sends its own session defaults (`single_kit_shared_state`, `auto_allocate_kit`) with `createReviewSession`. The viewer's `getReviewSession` and `getStreamConfig` threw a plain `Error`, so an A4 handoff whose revalidation read failed was rejected as `a4_handoff_request_failed` and marked retryable; they now throw `CoordinatorHttpError`, so the handoff is rejected with the reply's code (or `http_<status>`) and is retryable only for a 5xx, the rule consume and lease status already followed. Because the class is gone, six test files change in this bullet: `clients/coordinatorClient.close.test.ts`, `coordinatorClient.viewerCredentials.test.ts` and `a4Handoff.test.ts` move to the factory, `clients/viewerCredentials.test.ts` and `console/a4Handoff.dom.test.tsx` construct the single error type, and `console/windowParentMessage.dom.test.tsx` follows the renamed call. `scripts/verify-session-first-contract.mjs` follows the renamed call and the new source file, and the A1 outbox design evidence (`e2e/a1-outbox-design.spec.ts`) hashes the new source files. Former plain-`Error` failures now print as `CoordinatorHttpError: coordinator <path> -> <status> <detail>` and the queued conflict as `QueuedForInstanceError: coordinator /api/review-sessions -> 409 <detail>`; the console's message format itself is unchanged.
