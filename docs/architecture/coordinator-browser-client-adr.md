...
Amended on 2026-09-23 while implementing tracer bullet 1 (§4): the module is `web-viewer-sample/src/coordinatorClient/` (`errors.ts`, `transport.ts`, `routes.ts`, `client.ts`, `types.ts`, `index.ts`). `routes.ts` holds `COORDINATOR_ROUTES`, one entry per calling member with its method, contract path template and tag; the transport takes that route spec (`request(route, path, { body, headers, keepalive, signal })`) rather than a separate tag, method and `RequestInit`, so every call still sends the request shape its old primitive sent. `attempt` arrives with the CFD family in bullet 2, its first user. `coordinatorClient.contract.test.ts` checks every `operationId` tag against the contract and that every calling member has a route. `CoordinatorHttpError` also carries the parsed reply as `body`, which is how `createReviewSession` recognises the `queued_for_instance` conflict for every caller. An `error_code` that is not a contract code (lowercase snake case, at most 64 characters) is null, as the viewer already required; the viewer's `http_<status>` fallback moved into Window's A4 handoff detail. A 2xx reply that is not JSON is a 502 `coordinator_response_malformed`. The viewer's `closeReviewSession` had no production caller and demanded `status: "closed"`, stricter than the contract, so it is dropped and `sessionClose` serves the route. Window calls `streamConfig` (which encodes the session id) instead of `getStreamConfig` and sends its own session defaults (`single_kit_shared_state`, `auto_allocate_kit`) with `createReviewSession`. Because the class is gone, `clients/coordinatorClient.close.test.ts`, `coordinatorClient.viewerCredentials.test.ts` and `a4Handoff.test.ts` move to the factory in this bullet, and `scripts/verify-session-first-contract.mjs` follows the renamed call and the new source file. Former plain-`Error` failures now print as `CoordinatorHttpError: coordinator <path> -> <status> <detail>`; the message itself is unchanged.
