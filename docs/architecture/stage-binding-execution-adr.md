...
Amended on 2026-09-24 while implementing tracer bullet 1 (§3 step 1); §2 and §3 are read with this amendment. The module is `web-viewer-sample/src/stageBinding/` (`types.ts`, `execution.ts`, `index.ts`, `execution.test.ts`), created by `createStageBindingExecution(ports)`. This bullet implements the attempt lifecycle (`beginAttempt`, `supersedeAttempt`, `invalidateAttempt`, `terminalizeAttempt`, `isCurrentAttempt`, `isCurrentAttemptAwaitingProof`, `finishLoad`, `scheduleLoadTimeout`, `clearLoadTimeout`, `claimAttemptTimeout`), the Stage Proof (`revokeProof`, `blockProofAsChangedUnconfirmed`, `clearProofBlock`, `resyncProof`), `preauthorizeWithinDeadline`, `snapshot()` and `dispose()`; §2's `applyFromParent`, `apply`, `beginForAsset`, `receiveKitEvent`, `rejectCommand`, `observeVisibleFrame` and `onStreamReplaced` arrive with bullet 2. Eight ports are implemented rather than ten: `coordinator` (`preauthorize(artifacts, clientRequestId, signal)`, `cancel(clientRequestId)`, `revisions(sessionId, userToken)`), `session()`, `credentials()`, `tracker` (`attachStageAttempt`, `claimAttempt`), `nativeQueue` (`dropQueuedAttempt`, `invalidateQueuedAttempt`), `timers` (`setTimeout`, `clearTimeout`), `attempt` (`current`, `replace`, `advanceIntent`) and `view`. `send`, `mode()`, `post`, `onCompleted` and `review` are not needed yet: this bullet dispatches nothing, and every parent `stage_loaded` post and review-event line travels with a localized projection that stays in Window. `timers` has no `now`, because no bullet-1 operation reads the clock, and `revisions` takes no signal, because the Coordinator Browser Client's `getStageBindingRevisions` has none. A ninth port, `onState`, is declared optional and the module emits one snapshot per transition through it, but Window supplies no observer: it reads the module's own accessors live, so there is no second copy of this state to go stale. Bullet 3's A4 handoff and issue view, which read `snapshot()`, are the first consumers; `snapshot()` hands out a copy of the attempt record, not the live one.

What stayed in Window, and why. `_preauthorizeStageBinding` and `_cancelStageBindingPreauthorization` remain Window methods and serve as the `coordinator` port's `preauthorize` and `cancel`: 28 existing spies in `console/windowParentMessage.dom.test.tsx` pin their shape, and the second carries the 5 s cancellation deadline the ADR assigns to the module, so that one timeout stays in Window until bullet 3 rewrites those tests. The 45 s preauthorization deadline, the cancellation barrier and the 45 s stage-load deadline are the module's. `NativeStageDispatchQueue` does not move in this bullet: it needs `sendStreamMessage`, `failNativeOpenStageDispatch` and the stream generation, which are dispatch concerns arriving in bullet 2; the module fences it through the two-method `nativeQueue` port instead. `activeStageAttempt` and `stageIntentGeneration` stay as Window fields reached through the `attempt` port. `_applyBinding` remains a production writer of the intent counter (`++this.stageIntentGeneration`), and no production Window path assigns `activeStageAttempt` once the machine moved, so a delegating setter for it would exist only for the six fixtures in `console/WindowTraceAuthority.test.tsx` that install an attempt directly; that file plus `measurementWindow.dom.test.tsx` and `sectionPlaneWindow.dom.test.tsx` also advance the intent counter themselves. `pendingStagePreauthorizationIntent` stays in Window because no moved operation reads it — `advanceIntent` only clears it — while `_applyBinding`, the asset-open path and `_canApplyLoadingStateResponse` still own it. Localized copy, `AppState` writes and the parent posts stay in Window behind the ten-method `view` port (`loadedStageUrl`, `isLoadedStageExpected`, `finishLoad`, `attemptBegun`, `stageLoadFailureCleared`, `proofRevoked`, `bindingApplySuperseded`, `proofBlocked`, `stageLoadTimedOut`, `proofResynced`), because moving them would change what renders and in what order; `_finishStageLoad` is split, the module clearing the deadline and the pending target while Window clears the loading-state retry, the poll count and the first-frame latch. Window reads every former field through accessors that delegate straight to the module; `pendingStageUrl`, `confirmedStageBindingRevision`, `unprovenStageUrl`, `stageProofBlockGeneration` and `stageProofBlockedRevision` also delegate their setters, because the `_applyBinding`, asset-open and Kit result paths that still write them are bullet 2's. `dispose()` marks the module disposed, clears the stage-load deadline and abandons the active preauthorization, so a request waiting on the cancellation barrier ends as aborted instead of posting with an aborted signal; Window does not call it, because unmount does not abort a preauthorization today and wiring it would change that.

Two places in `console/windowParentMessage.dom.test.tsx` (four lines: each use and its local type declaration) follow the move instead of gaining a Window shim: the generation read becomes `stageBinding.attemptGeneration` and the timeout claim becomes `stageBinding.claimAttemptTimeout`, because Window itself no longer uses either. `Window.tsx` goes from 5993 to 5816 lines. Verified at this cut: `npx vitest run src/stageBinding src/console/windowParentMessage.dom.test.tsx src/console/WindowTraceAuthority.test.tsx src/console/a4Handoff.dom.test.tsx` (4 files, 355 tests), `npm test` (174 files, 2581 tests, from 173 and 2530), `npm run typecheck`, `npm run lint:baseline` (0 regressions), `npx playwright test e2e/runtime-command-authority.spec.ts` (6 tests) and `git diff --check`. The real Kit E2E on canonical Linux 181 that the Verification section requires for this cut is not run yet.
