...
Amended on 2026-09-24 after the review of tracer bullet 2: the coordinator's trace verification tells a trace it refused from one it could not check. It answers `200 {verified: false}`, which Kit drops, for a trace that is not the session's canonical trace, for a session the store does not find, and for its protocol refusals (`datachannel_trace_payload_invalid`, `datachannel_trace_header_missing`, `datachannel_trace_header_invalid`, `datachannel_trace_header_body_mismatch`). It answers `503`, which `RuntimeAuthorityClient` answers retryably like any non-200 answer, when it cannot establish the canonical trace: a store read or linked-trace lookup failure, inconsistent trace data (a malformed stored or linked trace, ambiguous linked roots, a conflict between them), or a failed commit (a stale plan, a failed backfill, a session gone between the plan and the commit). The preceding amendment's caveat that such failures are dropped no longer holds. Most reachable `503`s are persistent data inconsistencies, so a retry fails again, but the viewer gets an answer at once instead of waiting for its timeout, and it does not retry by itself. Both answers carry `detail_code: datachannel_trace_authority_unavailable`; the status tells them apart, and Kit logs both as a rejected trace. Two consequences are accepted. For a session whose file cannot be parsed or whose trace data is inconsistent, a foreign trace is answered too, so the answer tells an unverified caller that such a session exists; the caller must already reach Kit's DataChannel and name the session, and narrowing it would need the resolver to expose its candidate traces. A session file that `SessionStore.list()` has quarantined, a file that holds `null`, and an existence check that fails on an I/O error all read as a missing session and are dropped. The client accepts an answer whose echo is missing or names another trace only as an explicit `verified: false`; anything else without the echo stays retryable. A dropped command leaves the viewer to its own timeouts: a stage load waits 45 s, after which a manual artifact group load needs a stream reconnect before a retry, and embedded camera, section and overlay commands time out after 10 to 11 s.
