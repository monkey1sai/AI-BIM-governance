...
Amended on 2026-09-23 while implementing tracer bullet 1 (§5): the module is `src/services/cfdRunWorkflow/` (`workflow.ts`, `findingIssuePayload.ts`, `governanceIssueHttpAdapter.ts`, `index.ts`). In bullet 1 the streaming seam is `CfdRunPort = Pick<CfdRunClient, "getRun" | "getRunResult">` and there is no `conversionResults` dependency yet; bullet 2 widens the port and adds that dependency together with the operations that need them. The constructor also takes `log: Pick<StructLogger, "warn">` for the unbind warning of §Grilling ("a null store result is logged"). The not-ready outcomes carry `runStatus` and `sessionStatus`. `CoordinatorApp` does not expose the workflow: the HTTP routes exercise the wiring and the workflow owns no timers, so the repository's rule for `@internal` accessors (only where a wiring regression would otherwise go unnoticed) does not call for one. The per-run finding lock now drops its map entry when the last holder releases (the route-level lock compared against the wrong promise and kept one entry per run id). Since this ADR was written, S8 added `GET /api/cfd/options` and `POST /api/cfd/estimates`; they are pass-throughs that touch neither the ledger nor artifact URLs and stay in the routes until bullet 2 decides whether the workflow forwards them. Decisions §1, §3 and §5 are otherwise unchanged.

Amended on 2026-09-23 while implementing tracer bullet 2 (§5): create, list, detail, result, exclusions and cancel moved into the workflow, and so did the S8 pass-throughs (`getOptions`, `estimate`), so `registerCfdRunRoutes` takes only `{ enabled, workflow, rejectIfUnauthorized, authenticatePrincipal }` as §4 intended. `CfdRunPort` picks all eight client methods and `ConversionResultPort` is served by `StreamingConversionResultAdapter`, the only code that reads the client's `404:` message. Outcomes: `createRun` → `forwarded` | `conversion_not_found` | `source_not_ready` (carrying the conversion status) | `source_mismatch` | `unavailable`; `listRuns` → `records`; `getRun` → `detail` | `stale_record` | `forwarded` | `unavailable`; `getRunResult` → `result` | `forwarded` | `unavailable`; exclusions, cancel, options and estimate → `forwarded` | `unavailable`. `cancelRun` takes no principal (the route never passed one). A ledger write that fails while a reply is projected keeps the earlier mapping: `unavailable` on create and cancel, the cached record on detail, `stale` on list. The HTTP suite keeps one wire case per outcome; policy is tested at the workflow interface.
