...
Amended on 2026-09-23 while implementing tracer bullet 4 (§5): `CaseParams.end_time` is 600 and `build_parser` reads every `CaseParams` default from the dataclass; `PreprocessProfile.sealing_leak_fraction_limit` is 0.15; `run_preprocess` takes `leak_fraction_limit` (None = profile) and the service passes the request's limit through and reads the verdict back from `preprocess_stats.json` instead of recomputing it; `bimcfd preprocess --leak-limit` exposes the same knob. Tests pin the service's standard preset and the request schema defaults to those two sources. The §5 cutover is complete.
