...
Lane F/B 不自動啟動 ship-cycle。只有使用者明確要求 ship，或 Lane S 的已核准 spec 授權自主推進時，才使用 `.claude/workflows/ship-item.md`（commit→push→PR→local preflight→CI watch→buffered merge→closeout）。Lane G 預設停在 PR ready。完整 gate、reviewer buffer 與 finding fix contract 以 `ship-item.md` 為準。

自動化 `monkey1sai-blip` User/PAT approval lane 已退役。不得呼叫、安裝、修復或重建 protected broker，不得讀取 `BLIP_GITHUB_TOKEN`，也不得以 bot review、PAT、auto-merge 或 bypass 製造 counted approval。replacement authority 必須是 source-pinned、App-ID-pinned、exact-head 的 required AI CheckRun。

repo 內 `scripts/agent-tooling/blip-approve/` 僅保留歷史相容與 fail-closed 證據；所有 approval 入口必須回報 `HELD_AUTOMATED_APPROVAL_RETIRED`。現行 `.github/CODEOWNERS` 或 branch protection 若仍要求退役 identity，且尚未提供具 write 權限的人類 reviewer 或 GitHub team 名稱，回報 `HELD_REPOSITORY_APPROVAL_POLICY`，不得自行選人或放寬保護。

### Retired owner full-authority continuity（2026-09-04）

`$blip-approve` 現為 refusal-only compatibility skill；舊 continuity 不再授權任何自動 vote、broker 或 PAT 路徑。
...
### Owner standing merge decision（retired pending replacement reviewer）

在 replacement AI check 尚未完成 activation record 與 live protection 的 add-before-remove 遷移前，不得沿用 `monkey1sai-blip` approval 或普通 bot review 觸發 native merge；保持 `HELD_REPOSITORY_APPROVAL_POLICY`。
...
AI/bot replacement 必須透過 canonical activation record 指定的 source-pinned、App-ID-pinned required CheckRun，不能透過 GitHub review API 冒充 human/code-owner approval。live protection 完成 add-before-remove 遷移前保持 `HELD_REPOSITORY_APPROVAL_POLICY`。保留 strict non-empty required checks、enforce-admins、reviewer buffer、完整 protection/ruleset snapshot、immutable-SHA diff、final evidence re-read、exact-head REST merge與禁用 `--admin`／`--auto`；任一 protection/check/App/head/base 漂移即 HELD。
