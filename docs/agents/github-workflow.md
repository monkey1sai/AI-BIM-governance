...
本 repo 採 single-owner、dual-identity merge governance：同一位人類持有 owner 與固定 reviewer 兩個 GitHub 帳號，但 branch protection 保留 approving reviews=1。PR 作者不得自批；唯一有效 approval 必須由 `monkey1sai-blip`（immutable user ID `311287868`、type=`User`、association=`COLLABORATOR`）在 GitHub UI 提交 `APPROVED` review，實作會在 preparation 與 merge 前同時複驗該帳號 live permission/role 都精確為 `write`；review body 與 `commit_id` 精確綁定 repo、PR、base SHA、head SHA；elevated path 使用 `merge-elevated` action。Agent／GitHub App／bot 不得提交、修改或 dismiss 該 review，也不得以可重播的 commit status 冒充 PR-bound human approval。Branch protection 保留 dismiss stale reviews、conversation resolution、strict non-empty required checks、enforce-admins、90 秒 reviewer buffer、完整 protection snapshot、immutable-SHA diff、apex arbiter、final evidence re-read、`--match-head-commit` 與禁用 `--admin`；任一 protection/comment/review/reviewer permission/head/base 漂移即 HELD。canonical 格式、identity 與殘餘 credential trust boundary 見 `.claude/workflows/ship-item.md`。
