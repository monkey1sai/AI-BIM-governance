...
- [x] **Step 1.1: Build status command** (query open PRs, base/head SHAs, mergeable state, CI check states, review decisions).
- [x] **Step 1.2: Build update-branch & auto-resolve-conflict command** (auto-fetch origin/main, rebase PR branch, auto-resolve trivial docs/metadata conflicts, push).
- [x] **Step 1.3: Build auto-fix & approve command** (ensure compliant metadata table, call run_blip_human_equivalent_approve_once.ps1).
- [x] **Step 1.4: Build merge & process-queue command** (poll CI checks, execute squash merge, prune local/remote branches).

### Task 2: Create Canonical Skill & Align Manifest
- [x] **Step 2.1: Author .agents/skills/pr-queue-manager/SKILL.md**.
- [x] **Step 2.2: Update agent-skills-manifest.json and sync skills**.

### Task 3: Local Verification, PR, Review & Merge
- [x] **Step 3.1: Verify manage-pr-queue.mjs on active open PRs**.
- [x] **Step 3.2: Run local preflight and commit**.
- [x] **Step 3.3: Open PR, submit approval, pass CI, merge to main**.
