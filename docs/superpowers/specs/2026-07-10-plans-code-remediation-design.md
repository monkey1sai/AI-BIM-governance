...
- **收尾補遺（同日）**：compose.host-kit.yml 補 TEST_DATA_PROJECT_IDS 透傳——B2 的 R8 badge 在 docker web-plane 需 compose 環境白名單（.env 不掛進容器，與 MINIO_WATCH_* 同型）；部署區重建實測發現、值由 .env.web-plane.host-kit 注入。
