...
- **reference/gate trust boundary**：產品路徑與 manifest／golden reference authority 同 PR 變更一律 `reference_authority_mixed_fail_closed`；rebaseline 必須獨立 PR。workflow／producer／validator／rebaseline authority 要成為可信 merge gate，外部 branch protection必須要求可由 PR 作者以外至少一位合格 owner／CODEOWNER 滿足的 approval並撤銷 stale review，或改由可信 base-branch workflow驗證。若經明確授權採 solo-maintainer 例外，仍必須保留 Require PR、strict required checks、`enforce_admins=true`、無 bypass與禁止 force push／deletion；approval=0／CODEOWNER review=false只解除 merge deadlock，不建立獨立 review trust。只填 PR body或只存在 CODEOWNERS routing都不算獨立核准；實際遠端設定見 TRUTH §1。
- **可重現性邊界**：只有 resolved dependency snapshot、runner image與font fingerprints均經 machine-pin，visual run才具完全可重現性；任一項未達成時不得開啟 `full_completion_allowed` 或宣稱 production 99%。實際 pinning／enforcement 狀態見 TRUTH §1與 manifest。
