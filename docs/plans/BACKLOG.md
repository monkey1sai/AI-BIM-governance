...
> v4 · 2026-07-15 · 使用者裁決 A1–A4 閉環目標優先重排（a4-closeout → a3-clash → viewer 渲染兩項）；design dual-gate 體系（v3）不變。
...
|---|---|---|---|---|---|---|---|
| `gap-a4-closeout` | `#a4` | TARGET-shell `#a4` 節 | TRUTH §1 `a4` 列 | PARTIAL×高（**2026-07-15 使用者裁決：A1–A4 閉環第一執行位並承接體系 E2E 驗收角色**；同日 runtime 閘已交付——`:8005` 隔離站 trace/E2E 2 passed＋BCF 裁決落地，evidence=`artifacts/e2e/a4-trace/`） | **剩 `workspace.a4.default` visual gate**（兩 viewport pixel≤1%＋semantic 100%）；其餘 DONE 項（tracked browser trace `:8005`、E2E、BCF bridge 裁決＝**維持 unavailable 並於 UI 誠實標示**、不申請 contracts §1 #8 exception）已於 2026-07-15 達成 | 「建立 Issue (BCF)」維持 unavailable、不偽成功（2026-07-15 裁決＝不建 bridge）；confidence 僅在有定義／校正來源時顯示；Ornith vLLM 未連線時降級為 deterministic 且明示，不阻斷頁面、不偽稱 semantic | design visual gate 全站 fail-closed（`semantic_contract.status=state_variants_reference_missing`，TRUTH §1 `design_reference.snapshot` 段）；解鎖屬 `gap-design-system-production-alignment`，本 gap 不偷渡 |
| `gap-a3-clash` | `#a3` | TARGET-shell `#a3` 節（碰撞偵測 · Clash） | TRUTH §1 `a3` 列＋§3 #9 | PARTIAL×中（O6 已裁決：ifcclash；spec/plan 已備待開工；2026-07-15 使用者裁決＝A1–A4 序列第二位） | 依既有 spec/plan（`docs/superpowers/specs/2026-07-07-a3-clash-ifcclash-design.md`＋`docs/superpowers/plans/2026-07-07-a3-clash-ifcclash.md`）從 Task 0 runtime probe 起步，落地 ifcclash 碰撞檢測與 clash 端點；probe unavailable（含 `has_occ=False` 類原因）必加 hard guard，顯示機器可讀原因而非 0 碰撞；E2E | 實作前 clash 區零假數、`重跑碰撞檢測` 維持 disabled＋待建標；引擎 unavailable 態是有效 UX，不偽裝成功 | 無硬依賴；Task 0 安裝／smoke 若補不起幾何能力，依核准 plan 交付 unavailable hard-guard 模式 |
| `gap-viewer-m4-select-semantic` | `#viewer` | TARGET-viewer 區塊 1＋2（AC-1） | TRUTH §2 區塊 1／區塊 2 列 | PARTIAL×高（M4 核心迴路第一段；2026-07-15 使用者裁決＝A1–A4 序列第三位·demo 渲染） | 真串流畫面點選構件→高亮鎖定→右欄顯示 IFC Type／Name／GlobalId(22 碼)／PredefinedType／ObjectType／Tag／Fidelity；E2E | 串流遙測未取得標 n/a；無 GPU/harness 時 `KIT runtime=no`，不偽綠 | 無 |
| `gap-viewer-m4-a1-overlay-reverse` | `#viewer` | TARGET-viewer A1 疊加＋反向跳轉（AC-6/AC-7） | TRUTH §2 區塊 1 列＋§1 `a1` 列 | PARTIAL×高（M4 收尾；2026-07-15 使用者裁決＝A1–A4 序列第四位·demo 渲染） | A1 疊加 ON→選定檔 rule-run 失敗構件 3D 紅色高亮（資料源 governance :49102 經 coordinator proxy）；對應表列／問題列／結構樹三入口反向跳轉；`highlightPrimsRequest` 經 DataChannel，**Kit 回 ack 才標成功** | 證據未齊時高亮鍵維持 disabled；ack 未回不標成功 | 四證據鏈缺一不可：DataChannel ready ∧ first_frame_at ∧ stage matched ∧ usd_prim_path（現況見 TRUTH §2 區塊 1 列） |
| `gap-design-system-production-alignment` | `#home/#a1–#a10/#conv/#runtime` | contracts §5.1＋TARGET-shell 對應節 | TRUTH §1 design fidelity 欄 | approved baseline 與可用 machine protection 已有、獨立 review trust 尚缺、production 未實測×最高 | `implemented_case_ids` 精確等於 11 個 `required_case_ids`；Playwright 對 current subject commit 產出全部 13 screens×2 viewport pixel≤1%＋semantic 100% result；workflow producer landed 後依既有授權啟用 `design-semantic-visual` required context；每 route functional/runtime gate另過 | concept golden 不代表 A5–A10 backend 已建；design pass 不改 Prov/建成狀態 | semantic state variants 尚缺、producer change 尚未 landed、required context暫緩；solo-maintainer 模式不要求人工核准，product gate fail closed |
...
| `gap-design-gate-trust-determinism` | 全部 frontend／design gate infrastructure | PROCESS §3 | solo-maintainer protection 已保留 Require PR、strict 11 checks、admin enforcement、無 bypass與禁止 force push／deletion，並設定 approval=0／CODEOWNER review=false；machine merge path 可用但沒有獨立人工核准；design context待 producer landed；npm dependency tree／runner image／font fingerprints未 pin | 獨立 review authority 尚缺且 pixel run不可完全重現×最高 | 新增至少一位獨立合格 CODEOWNER／協作者並重新啟用可滿足的 owner approval，或落地可信 base-branch gate；land trusted workflow producer後依既有授權啟用 required context；保持 reference-only rebaseline 與 product PR 分離；resolved dependency snapshot、runner image與font fingerprint machine-verified | approvals=0 不等於取消 PR／required checks，也不等於獨立 review trust 已建立；required context不得先於 producer／executable gate；完成前維持 `full_completion_allowed=false` | 第二 reviewer／trusted base-branch authority 尚缺；仍需完成 merge→activation順序與裁決 lock/snapshot policy |
...
**尚未排入 gap 聲明**：A5–A10 的完整目標已在 TARGET-shell 對應節；現行 NOT BUILT／disabled 佔位是誠實的 runtime 現況，不是最終需求。A4 已於 2026-07-14 以 deterministic search vertical slice 接入 B 閉環（ownership＝governance-service + coordinator proxy；見 §2 #7 裁決）。其餘頁在選定 vertical slice 前不列入 §1。`#admin` 與 kit-manager-api GPU/心跳遙測（TRUTH §3 #6，IX-SS-02 前置）同樣尚未排入 gap，UI 維持 disabled／「未取得」；遙測一旦立項即同時解鎖 `gap-ss-03-force-release` 的 blocker。ChatUSD agent 真對話／MCP 執行（TRUTH §3 #2）：2026-07-15 使用者已選定為 Kit MCP 切片方向（詳 §2 #7 A10），ownership 裁決落定前仍不進 §1、UI 維持 input disabled。
