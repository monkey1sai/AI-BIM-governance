...
> 分工：現況問 `TRUTH.md`；需求問 `TARGET-shell.md`／`TARGET-viewer.md`（視覺錨＝兩份 prototype HTML）；全域不變量問 `TARGET-contracts.md`；驗收與紀律問 `PROCESS.md`。本檔只管「還缺什麼、先做哪個、哪些決策還開著」。
...
依排序。每 gap 的 DONE 至少含：該頁 screenshot/E2E 與 prototype 錨對齊＋TRUTH 對應列同 PR 更新（PROCESS §2/§3）。
...
**不開 gap 聲明**：A4–A10 與 `#admin` 不開 gap——NOT BUILT 佔位頁（disabled、不接 mock、phase 佔位）已是 prototype 要求的最終姿態，現況由 TRUTH 各記一列即可；後端何時開工屬產品決策，屆時再加列。ChatUSD agent 欄真對話／MCP 執行（TRUTH §3 #2）與 kit-manager-api GPU/心跳遙測（TRUTH §3 #6，IX-SS-02 前置）同樣不開 gap——均屬 p1 產品決策範圍，UI 維持 disabled／「未取得」誠實姿態，屆時再加列；遙測一旦加列即同時解鎖 `gap-ss-03-force-release` 的 blocker。
