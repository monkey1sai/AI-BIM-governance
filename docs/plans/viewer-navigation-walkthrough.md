...
- **速度 1 等於步行速度（1.4 m/s，容許 ±10%）**：Kit 內建飛行不依場景單位換算，原設定在公尺場景下速度 1 約每秒 415 公尺。每次開啟場景時，Kit 依 `metersPerUnit` 改寫官方設定 `/persistent/app/viewport/manipulator/camera/flyAcceleration`，公尺場景約為 3.371。換算依據：按鍵輸入為 5 × 速度、阻尼 10、每格時間上限 0.0166 秒、串流主迴圈 60 Hz。
...
| V3 | 飛行速度的官方設定鍵 | 已確認設定可套用：`/persistent/app/viewport/camMoveVelocity`，設 3 後回報 3（A5）。速度 1 已校正為步行速度（見 §6），實際前進距離待 V1 以真實按鍵量測 |
