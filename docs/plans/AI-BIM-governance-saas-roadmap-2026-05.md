...
| `streaming-ifc-usdc-conversion-authority` | 1/4 | 4 | ✓ `bim-streaming-server` 成為 IFC→USDC conversion job / status / result authority；可解析 IFC 遇到 HOOPS import/load failure 時可走 scoped fallback，但仍必須通過 publishable artifact gates |
