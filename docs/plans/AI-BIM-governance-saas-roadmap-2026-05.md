...
>
> **2026-06-02 更新（`author-ifc-openusd-identity-paths` archive 對齊；IFC-first OpenUSD identity authoring profile）**：本 change（implementation PR #149，2026-06-02 merged，merge commit `b612bb7`）已完成 **OpenSpec sync/archive**（archive folder `openspec/changes/archive/2026-06-02-author-ifc-openusd-identity-paths/`；`openspec/specs/` 維持 **32**：`streaming-ifc-usdc-conversion-authority` ADD 3 requirements，要求 `conversion_profile = "ifcopenshell_openusd_identity"` explicit route 直接用 IFC + OpenUSD author stage、以 IFC `GlobalId` 產生 `/World/Elements/<IfcType>/G_<encoded_guid>` stable element root、輸出 `guid_exact` mapping 與 `entity_index` / `pset_index` / `spatial_index` / `bbox_index` / `geo_reference` artifact refs，並維持 callback outbox metadata-only 與既有 conversion profile 相容）。**Evidence**：`docs/evidence/author-ifc-openusd-identity-paths/` 收錄 `stream_conv_20260602080050_a0cc1bdc` identity smoke，`mapping_fidelity = "guid_exact"`、`coverage_ratio = 1.0`、`unmapped_count = 0`、`minimum_coverage_baseline_locked = false`，stage open result / quality metrics / mapping sample / sidecars 皆留檔。**Review note**：外部 `$gitnexus-pr-review` 對 PR #149 留 2 medium + 1 low 觀察（profile preflight、coverage/metrics 語意、local absolute paths evidence），使用者已同意先 merge -> archive -> 收尾；後續若要把 identity profile 變成 default 或 production baseline，需另開 change 補 preflight/metrics/evidence hardening。本 archive 不升等 Docker GPU launcher、dedicated multi-Kit、OQ1 callback auth 或 OQ5 SSO。
>
> **2026-06-03 更新（批次歸檔 9 個對抗驗證修復 change；A1/A2/A3 + edge-console + deploy 治理硬化）**：本批為 governance / edge-console / federation / deploy 既有 capability 的「**對抗驗證 + 修復**」change，共 9 個（implementation PR #164–#172，皆 2026-06-03 merged 進 `origin/main`），於本輪一次 `npx openspec archive` 歸檔（archive folder `openspec/changes/archive/2026-06-03-<id>/`，溯源見 §1.4）：① `a1-rule-engine-honesty`（#164，A1 rule-engine + IDS 誠實計分／彙總，7 findings）、② `issue-bcf-integrity`（#165，Issue-DB 完整性 + BCF 匯出 provenance，11 findings）、③ `a2-diff-tag-typeguard`（#166，A2 diff Tag 級型別護欄 + 穩定配對，3 findings）、④ `a3-federation-trs-coords`（#167，A3 federation 標準 TRS + 座標一致性，4 findings）、⑤ `edge-console-type-prov`（#169，Edge Console 型別正確性 + provenance 誠實對齊，tsc 11→0，5 findings）、⑥ `deploy-nullderef-guard`（#168，`deploy.ps1` Final Summary／signature null-safe，2 findings）、⑦ `fix-federation-resetxformstack`（#170，federation 保留 member `!resetXformStack!` 時 transform 仍套用，merge 後 reviewer P1）、⑧ `frontend-coordinator-env`（#171，console governanceClient 用正規 `VITE_COORDINATOR_API_BASE`，HIGH）、⑨ `governance-adversarial-followups-2`（#172，最終對抗複驗第二層 IDS／diff／issues／BCF 誠實與正確性，7 findings）。本批**不新增 capability**（39 → 39），只 MODIFY／ADD requirement 到 2026-06-03 followup 批引入的既有 governance/edge-console/federation/diff capability。**合併多 change delta 的 capability（本輪手動處理，避免較早 change 的規格意圖被較晚 change 的 MODIFIED delta 覆蓋）**：(a) `usd-federation-sublayer-sets`「per-member transform」requirement 由 ④`a3-federation-trs-coords`（標準 TRS：translate 最外層／scale 最內層，pxr 世界座標驗證）+ ⑦`fix-federation-resetxformstack`（leading `!resetXformStack!` 維持 index 0、fed ops 置於 reset 之後）**合併**，最終含 4 scenarios；(b) `model-version-diff-authority`「GlobalId 多級對齊」requirement 由 ③`a2-diff-tag-typeguard`（多級退階維持型別一致性、跨型別共用 Tag → removed+added）+ ⑨`governance-adversarial-followups-2`（同 `(ifc_type, Tag)` 各側恰 1 個才以 Tag 配對、杜絕幻影 moved）**合併**，a2 另一 ADDED「同鍵簇配對穩定可重現」requirement 保留；(c) `governance-issue-tracking`「可由 rule-run/diff 來源批次產生」requirement 由 ②`issue-bcf-integrity`（單一交易全有或全無 + 冪等 skipped 計數）+ ⑨（rule-run best-effort 版本綁定 / diff 缺 target_model_version_id 回 422 的刻意不對稱）**合併**，最終含 4 scenarios；(d) `governance-bcf-export`「匯出為 BCF 2.1」requirement 由 ②（22 字元合法 IfcGuid 才匯出、`unbound` 佔位字、naive 時間視為 UTC）+ ⑨（F5 誠實校正：匯出模組執行期不 import bcf-client，惟 ifctester transitive 安裝 GPLv3 bcf-client）**合併**，最終含 7 scenarios；(e) `governance-rule-run-authority` ①`a1-rule-engine-honesty`（誠實計分公式 passed/(passed+failed+errored)、errored 計入分母 + ADD「IDS 彙總唯一可辨識／error 計數誠實」）與 ⑨（IDS 跨 model 不殘留假通過、required spec 零適用構件誠實 fail）改的是**不同 requirement**，無需合併但兩者規格意圖皆已併入。**Verification（誠實，依 §1.6／§0.1 不標 §1.3 runtime passed）**：本輪 archive 後 `npx openspec validate --all --strict` = **39 passed / 0 failed**、`npx openspec list` 無 active change；各 implementation PR 於 merge 前已過 pytest／openspec／對抗複驗（findings 數見上）。**本 archive 僅代表「規格已併入現行 `openspec/specs/` + 程式已於各 PR 修復並通過當時 CI」，未取得新的 runtime／E2E evidence，因此不把任何 §1.3 閉環或 runtime tier 標成 passed**（governance-service 真機 IFC rule-run／diff／federation／BCF 匯出 + Edge Console 瀏覽器 E2E 仍待 host-native runtime 證據）。本 archive 不升等 Docker GPU launcher、dedicated multi-Kit、OQ1 callback auth 或 OQ5 SSO；不改任何對外 API／contract（各 PR Non-goals 已聲明）。
>
> **2026-06-03 更新（`edge-console-a1a3-interface-completion` #174 + `agent-entry-boundaries` #175 archive 對齊）**：兩 merged change 經 `npx openspec archive` 歸檔（archive folder `2026-06-03-edge-console-a1a3-interface-completion/`、`2026-06-03-agent-entry-boundaries/`）。`edge-console-operator-frontend` **+3 requirements**（A1 真實 Excel 匯出 + 誠實 p1 3D 標示入口、A2 apply-overlay 誠實回 501 不偽裝成功、A3 build-time member visibility 並誠實標須重新 Build）並補該 spec `## Purpose`（原 archive 遺留 TBD）。**新增 capability `agent-operability-governance`**（#175，3 requirements：agent 邊界對齊 A1–A10 產品定位、user-facing 完成 SHALL frontend-operable、deploy 行為 SHALL 走 canonical scripts）→ `openspec/specs/` **39 → 40**。`npx openspec validate --all --strict` = **40 passed / 0 failed**、`npx openspec list` 無 active change。**Verification（誠實，依 §0.1/§1.6）**：本 archive 僅代表規格已併入現行 `openspec/specs/` + 程式已於 #174/#175 merge 前過 CI（pr-review-agent + CodeRabbit），未取得新 runtime/E2E evidence，不把任何 §1.3 閉環或 runtime tier 標 passed。
>
> **2026-06-02 更新（`harden-internal-auth-and-config-hygiene` archive 對齊；L4 第一批 internal-auth/config 防禦固化 5 項）**：本 change（implementation PR #148，2026-06-02 merged，squash `a0c7ece`）已完成 **OpenSpec sync/archive**（archive folder `openspec/changes/archive/2026-06-02-harden-internal-auth-and-config-hygiene/`；`openspec/specs/`：`host-native-conversion-authority-service` **MODIFY「exposes internal API」**（加 GET-over-loopback + token-unset demo scenario）+ `one-click-deploy-hybrid` **MODIFY「Mode C hybrid 一鍵部屬入口」**（加 deploy.ps1 env fallback warning + missing-example throw scenario），淨 0 requirement）。本 change 來源 = L4（需業務/資料拍板那批）經 2026-06-02 **brainstorming** 拆三類後的第 1 類（可設定防禦）+ 第 2 類（repo 治理）收斂；design doc `docs/superpowers/specs/2026-06-02-harden-internal-auth-and-config-hygiene-design.md`（已標 SUPERSEDED 指向本 archive）。**關鍵發現**：explore 讀 code 確認 #2/#6 防禦機制**已存在**（#2 `conversion_authority.py:98-104`、#6 `app.ts:766-776` + `isInternalRequestAuthorized`），真實工作 = 固化既有行為 + 補可觀測 test + 加收緊旋鈕 + 文件，**零既有 production 預設行為改動**（複用 CH-2 strict pattern）：#2 token enforce 補 test（403 / env 讀取 / 預設 None demo regression）+ GET loopback comment（使用者拍板 GET 不加 token）；#6 internal middleware 白名單（`/viewer-log`、`/structLog/health`）刻意 unauth comment + `internalAuth.test.ts`（5 cases，使用者拍板維持 unauth）；#36 退役 collaboration event 過濾 `eventLogFilter.test.ts`（listLifecycle 排除、list 全量 archive-compat）；#26 kit-manager-api CORS 改可設定 `KIT_MANAGER_CORS_ORIGINS`（預設 `['*']` 不破 + compose env 接線）+ test；#23 `deploy.ps1`+`check-web-plane-docker.ps1` env fallback `Write-Warning` + 連 `.example` 都無 `throw`（使用者拍板；**不碰 spec 保護的 `start-web-plane-docker.ps1`**）。**Verification**：streaming pytest **84**、coordinator npm verify **271**、kit-manager pytest **11**、deploy dryrun + env-fallback **ALL PASSED**、root pytest **66**；`openspec validate --strict` ✓。**GitNexus pre-impact**：kit-manager `Settings`/`create_app` upstream **LOW**。**Review loop（兩輪）**：opus 對抗式 4-lens **全 LGTM、零 P0/P1**（correctness 逐句核對 comment 無杜撰、#23 三段式 env 解析嚴密、#6 allowlist path-mount 正確；security 確認無 CORS credentials 漏洞 + 無意外 unauth + env 無機密；spec-doc 確認 #23 沒碰 `start-web-plane-docker.ps1`）；外部 **Codex P1** 抓 **#29 違反 archive immutable**（`openspec/AGENTS.md` L30：改 archive 需獨立 historical-correction PR）→ #29 evidence 搬移**移出本 change**、改走獨立 PR；**Copilot** 抓 #26 `_parse_cors_origins(",")`→空 list（→ fallback `['*']`）+ comment 措辭；**Codex P2** #26 compose env 未接線 → 補 `KIT_MANAGER_CORS_ORIGINS`；皆 fix。**Defer follow-up**：#29 evidence→archive sibling（獨立 historical-correction PR）；`start-web-plane-docker.ps1` fallback 警告（需 MODIFY spec）；#2 GET 保護（非 loopback 部署）；`check-web-plane-docker.ps1` throw（diagnostic 設計選擇）；#21/#33（L4 第 3 類另議）；#34（branch git 整理）。CI：CodeRabbit + pr-review-agent passed（兩輪）。**注意**：不改既有 production 預設行為（#2 預設 None、#6 白名單 unauth、#26 預設 `['*']`）；不碰 `start-web-plane-docker.ps1`；不新增 production dependency。此 archive 未升等任何 §1.3 runtime tier 為 passed。
>
> **2026-06-02 更新（`tidy-repo-scripts-tests-hygiene` archive 對齊；repo/scripts/tests 衛生清理 8 風險）**：本 change（implementation PR #147，2026-06-02 merged，squash `afae2b7`）已完成 **OpenSpec sync/archive**（archive folder `openspec/changes/archive/2026-06-02-tidy-repo-scripts-tests-hygiene/`；`openspec/specs/`：`documentation-source-of-truth` **ADD 1 requirement**（Superseded design drafts SHALL point to the authoritative archive）+ `one-click-deploy-hybrid` **MODIFY「Final Summary 可診斷性」**（加 build/probe 失敗仍達 Print-FinalSummary、不得 strict-mode crash 的 scenario），淨 +1 requirement）。本 change 來源 = 2026-06-01 風險報告 CH-5 分組（risk-triage 第五個 change，收 #22/#25/#31/#37/#39/#20/#30/#40，全衛生）：explore 實證後判定**唯一真風險是 #22**——`scripts/deploy.ps1` 在 `Set-StrictMode -Version Latest` 下 build/probe 失敗路徑的 `$kitBuildExit`/`$buildExit`/`$runningIds` 未賦值就被讀取 → `VariableNotDefined` terminating crash 繞過 Print-FinalSummary（三處各加 `=-1`/`=@()` fail-safe 初始化）；#25 抽 `$script:MaxSpectatorCount=32` 單一 source（維持 32，使用者拍板）；#31 root `.gitignore` 補 `bim-streaming-server/*.etl`（defence-in-depth，已被 sub-repo gitignore 主覆蓋）；#37 新增 `tests/conftest.py` 集中 sys.path、刪 `test_contracts_and_fakes.py` per-file insert；#39 fake worker `urlopen` 加 `_ALLOWED_HOSTS`（localhost 家族，使用者拍板）host 白名單 + host-reject test；#30 `OutputNamne` alias 加勿改 comment（刻意向後相容）；#20 `app.ts` `pendingDispatchEvents.set` INVARIANT 註解（Node 單執行緒非真 race）；#40 舊草稿加 SUPERSEDED 指向 archive。**Verification**：Python `pytest tests` = **66 passed**（baseline 65 + 1 host-reject）；TS `bim-review-coordinator npm run verify` = **264 passed**（#20 comment-only 0 regression）；PowerShell `test-deploy-dryrun.ps1` = **ALL PASSED**、`test-convert-ifc-to-usdc.ps1` passed（#30 alias 映射不變）；`git ls-files "*.etl"` 空；`openspec validate --strict` ✓。**GitNexus pre-impact**：`pendingDispatchEvents`（唯一 indexed，comment-only impact nil）；deploy.ps1 / .gitignore / docs / python tests 非 indexed symbol。**Review loop**：opus 對抗式 4-lens（純文字）**全 LGTM、零 P0/P1/P2**（correctness 確認 #22 真 latent bug + #39 host 白名單 edge case hostname=None/IPv6/大小寫全正確；security 零 finding；regression 66 passed；spec-doc archive 路徑真實 + delta 一致）；外部 Copilot（host-reject test base_url 含 path → 改純 host）+ CodeRabbit（deploy.ps1 無 BOM 中文 = pre-existing 51 行，未惡化）皆處理。**Defer（pre-existing / 超 scope）**：deploy.ps1 無 BOM + 中文註解（既有編碼慣例，加 BOM 動全檔超衛生 scope）；`.etl` 物理刪除與舊草稿刪除（預設不刪）；deploy.ps1 拆檔模組化（另立 refactor change）。CI：CodeRabbit + pr-review-agent passed（兩輪）。**注意**：不做 production 行為改動；不改 32 數值 / `$ErrorActionPreference` / `OutputNamne` 拼錯 / #20 race 邏輯；不移除 `# noqa: S310`；不新增 production dependency。此 archive 未升等任何 §1.3 runtime tier 為 passed。
>
> **2026-06-02 更新（`harden-web-viewer-test-resilience` archive 對齊；web-viewer 引入 vitest 測試骨架 + 8 健壯性風險修復）**：本 change（implementation PR #146，2026-06-02 merged，squash `73749c7`）已完成 **OpenSpec sync/archive**（archive folder `openspec/changes/archive/2026-06-02-harden-web-viewer-test-resilience/`；`openspec/specs/` 維持 **32**：`session-first-review-viewer` **MODIFIED 3 requirements**（Viewer bootstraps：加 spectator-binding-by-primary-id + bim-control-cannot-bypass-coordinator scenarios；handles lifecycle transitions safely：加 poll-bounded + stream-teardown + background-timer-clear + sdk-load-failure scenarios；displays streaming-owned status：加 spectator-trusts-primary scenario），0 ADD / 0 REMOVED — capability 數不變）。本 change 來源 = 2026-06-01 風險報告 CH-4 分組（risk-triage 第四個 change，收 #17/#27/#8/#28/#15/#16/#18/#32，全純前端 `web-viewer-sample`）：#17 引入 `vitest`+`jsdom`（devDep）+ 獨立 `vitest.config.ts` + 抽 `windowHelpers`/`pollHelpers`/`envHelpers`/`triReady` 純函式 colocated test（**34 passed**）；#27 `_pollForKitReady` 存 `_pollForKitReadyId` + `componentWillUnmount` clear（進入點改 `_clearPollForKitReady()` 閉合 in-mount 重入孤兒 chain）；#8 `AppStream` 兩處私有 `(AppStreamer as any)._stream=null` hack 換公開 `terminate(false)`；#28 `pollForSessionReady` 加 `MAX_POLL_RETRIES=36` 上限 + 逾時 `_resetState(connectionText)` + catch reschedule；#15 `selectSpectatorBinding` 用 `viewport_sharing.primary_kit_instance_id` 顯式挑（先驗 id 在 bindings 內否則 port-diff fallback）；#16 spectator 僅在 `viewport_sharing.spectator_ready` 時 `stageLoadStatus='matched'` 否則 pending；#18 `config/env.ts` 移除 `VITE_BIM_CONTROL_API_BASE` override（`resolveBimControlBase` 恆等 coordinator）；#32 刪 `index.html` 無條件 GFN script → `AppStream` `source==='gfn'` 動態載入 + `globalThis.GFN` 讀取 + `onerror` 回饋。**Verification**：web-viewer `npm run verify`（vite build + vitest **34 passed** + struct-log **10 passed**）+ `test:session-first`（含 5 source-level 契約斷言，因 #17 抽函式改查 Window.tsx+windowHelpers 聯集）；root pytest `tests` = **65 passed**（0 regression）；`openspec validate --strict` ✓。**GitNexus pre-impact**：`pollForSessionReady` / `_resolveStreamEndpoint` / `_pollForKitReady` / `_resetState` upstream 全 **LOW**（d=1 caller 皆同檔、不改簽章；web-viewer-sample 是主 `AI-BIM-governance` index 子目錄、非獨立 index）。**Review loop（雙層多輪）**：opus 對抗式 5-lens（純文字）抓 2 P2（#27 in-mount 重入孤兒 chain、#32 remount-during-load GFN ReferenceError）+ 2 P1 doc（design SDK 簽章捏造 `5.6.0/L44/_force`，實裝 `5.17.0/L71/terminate(terminateApp?)`；tasks/design 承諾 test 未交付 → 補 `triReady.test.ts`）；外部 Copilot/Codex/CodeRabbit 補抓（#32 `_initStream` 裸 `GFN`→`globalThis.GFN`、#15 id 不在 bindings 誤選 primary、#16 該驗 `spectator_ready`、envHelpers null case、vitest.config 分號）；皆 fix。**Defer follow-up（review 揭露）**：reviewRequestId+`BimControlClient` legacy 路徑打 coordinator 無的 `/api/review-session-requests` 會 404（既有 gap，非 #18 引入：`VITE_BIM_CONTROL_API_BASE` 未在任何 deploy 設定、#18 前後預設一致、主路徑 session-first 不受影響）；#28 逾時未 `destroyStreamingSession`；#16 spectator readiness 不 refresh；`_reconnectStream` 未 await terminate；GFN unmount guard；`BimControlClient→ReviewMetadataClient` 改名（跨 repo / 超 spec scope）。CI：CodeRabbit + pr-review-agent passed（三輪）。**注意**：不改 `bim-review-coordinator` / `bim-streaming-server` / 任何對外 contract（coordinator REST / Socket.IO / DataChannel JSON / callback payload）；vitest/jsdom 只進 devDependencies、不新增 production dep；不升級 `@nvidia/omniverse-webrtc-streaming-library`（pin ^5.6.0、實裝 5.17.0）；既有 4 個自製 `.mjs` verify 與 30 個 pre-existing ESLint errors 不在 scope。此 archive 未升等任何 §1.3 runtime tier 為 passed。
>
> **2026-06-02 更新（`harden-stage-host-allowlist` archive 對齊；stage host 設定化 + 移除退役 8005 死碼 + SYSTEM_DESIGN as-built）**：本 change（implementation PR #144，2026-06-02 merged，commits `a0ed59f` + `7914c3b`）已完成 **OpenSpec sync/archive**（archive folder `openspec/changes/archive/2026-06-02-harden-stage-host-allowlist/`；`openspec/specs/` 維持 **32**：`one-click-deploy-hybrid` ADD 1 requirement（Stage allowed-hosts SHALL be configurable with an explicit empty-value warning），0 MODIFIED / 0 REMOVED — additive）。本 change 來源 = 2026-06-01 風險報告 CH-3 分組（risk-triage 第三個 change，收 #12/#24/#14）：#12 HTTP stage allowed-hosts 設定化 + 空值告警（`stage_loading.py` 的 `_http_stage_allowed_hosts` 空值/純空白（`raw.strip()`）走 warn+default 不靜默；`start-streaming-server.ps1` 加 `-AllowedStageHosts` param + 三分支 param→env→Write-Warning+default）；#24 移除三處退役 `_worker` :8005 死碼（`stage_loading.py` `_DEFAULT_HTTP_STAGE_ALLOWED_HOSTS` / `start-streaming-server.ps1` / `deploy.ps1` `Resolve-AllowedStageHosts` / README，只留 `127.0.0.1:49101` / `localhost:49101`）；#14 `SYSTEM_DESIGN.md` 改寫 as-built（新增 As-built 段 + §5-13 前瞻逐段標 `[DEFERRED]` + 補實際描述，§8 涵蓋 LoadingManager + StageManager 完整命令群；§3/§9 target sizing 保留加註，roadmap §9 引用）。**Verification**：pytest `test_stage_loading_stage_composition.py` = **3 passed**（baseline 1 + 2 新：env-driven / 空值 fallback 不含 8005）；root pytest `tests` = **65 passed**（0 regression）；PowerShell `test-deploy-dryrun.ps1` = **ALL PASSED**（含 #24 Test 10 clean-env allowlist drops :8005）；`test-stage-loading-contract.ps1` passed；`openspec validate --strict` ✓。**GitNexus pre-impact**：`_http_stage_allowed_hosts` upstream **LOW**（d=1 `_ensure_allowed_http_stage_url`，全在 stage_loading.py，0 cross-service）。**Review loop**：opus 對抗式 review（純文字 4 lens）抓 1 **doc-major**（SYSTEM_DESIGN §8 as-built 命令清單只列 LoadingManager 子集、漏同 extension `StageManager` 7 incoming + outgoing）+ correctness minor（whitespace-only env 靜默空集）+ 數個 nit；皆 fix（§8 補兩 handler group、`raw.strip()`）。CI：CodeRabbit + pr-review-agent passed。**Defer（既有設計 / scope 邊緣）**：deploy.ps1 vs start-streaming-server.ps1 內建 default 各自寫死 49101（去重不在 scope）；PS / Python warn 文案語言不一致。**注意**：不改 stage 載入核心 / host 強制檢查語意 / env 變數名 `BIM_REVIEW_STREAM_ALLOWED_STAGE_HOSTS`；不實作 SYSTEM_DESIGN 前瞻設計；不刪 §3 / §9 target sizing 數字（roadmap §9 引用）。此 archive 未升等任何 §1.3 runtime tier 為 passed。
>
> **2026-06-01 更新（`harden-coordinator-ifc-intake` archive 對齊；coordinator IFC intake 防護 + graceful dispose + 退役死碼清理）**：本 change（implementation PR #142，2026-06-01 merged，squash `648f495`）已完成 **OpenSpec sync/archive**（archive folder `openspec/changes/archive/2026-06-01-harden-coordinator-ifc-intake/`；`openspec/specs/` 維持 **32**：`local-coordinator-ifc-ready-intake-boundary` ADD 1 requirement（IFC-ready download honors explicit strict mode）+ `project-risks-mitigation` ADD 1 requirement（graceful shutdown wired to SIGTERM/SIGINT），0 MODIFIED / 0 REMOVED — additive）。本 change 來源 = 2026-06-01 風險報告 CH-2 分組（risk-triage 第二個 change，收 #9/#7/#19）：#9 IFC `fallbackOnFetchError` strict 接線（config 加 `ifcDownloadStrict` 讀 `IFC_DOWNLOAD_STRICT`，code default false 不破壞 demo；app 接 `fallbackOnFetchError=!strict`；compose ×2 加 `IFC_DOWNLOAD_STRICT: ${IFC_DOWNLOAD_STRICT:-false}` 讓 env 可覆蓋 + production 註解）；#7 graceful dispose 接線（`index.ts` 接 SIGTERM/SIGINT → `createGracefulShutdown`（dispose drain → io.close → server.close → exit 0；io 先於 server 避免 Socket.IO keep-alive 死鎖；dispose 包 try/catch），補實作 `RISK-IN-MEMORY-QUEUE-PERSISTENCE` graceful shutdown）；#19 徹底刪退役 `_bim-control` 死碼（刪 `BimControlClient` + `safeArtifacts` + config `bimControlApiBase` + compose `BIM_CONTROL_API_BASE`，保留 `Artifact` type 與 `buildArtifactBindings` / `buildStreamConfig` 簽章，清 11 個 test makeApp）。**Verification**：coordinator `npm run verify` = **264 passed**（baseline 257 + 7 新：#9 strict 502 / non-strict fallback、#7 shutdown 順序 ×3、config strict ×2；tsc 0 error = #19 型別清乾淨 + Artifact 簽章保住）；root pytest `tests` = **65 passed**（0 regression）；`openspec validate --strict` ✓。**GitNexus pre-impact**：loadConfig / downloadIfcToSharedVolume / safeArtifacts / createCoordinatorApp 等 upstream 全 **LOW**，0 cross-service impact。**Review loop（兩輪）**：opus 對抗式 review（純文字，4 lens）抓 4 major（#7 shutdown io/server 嵌套死鎖、#9 strict test 沒 pin reason、#9 non-strict fallback 零覆蓋、#7 signal handler 無測試）；外部 CodeRabbit + Codex 補抓（CodeRabbit major：dispose 未 guard 會中止 shutdown；Codex P2：compose hard-code false 讓 env 無法覆蓋；繁中規範）。**Defer follow-up**：(1) strict 對 non-http IFC ref（`minio://`）未生效（non-http 真下載未實作，超出 #9 http fallback scope）；(2) graceful shutdown in-flight intake race（SIGTERM 時已 accept 仍 download 的 request 在 drain 後才 enqueue 可能漏標，屬既有 dispose 時機、#7 只補 signal 接線未惡化，完整解涉及 server.close keep-alive 死鎖權衡）。CI：CodeRabbit + pr-review-agent passed（pr-review-agent 首跑 transient GitHub API ResponseEnded、re-run 後 pass）。**注意**：不做 ConversionDispatchQueue 持久化（spec 明文另立 change）；不復原 `_bim-control` discovery；不改 callback outbox；不引入新 production dependency；不移除 Artifact type；不改對外 API / Socket.IO contract。此 archive 未升等任何 §1.3 runtime tier 為 passed。
>
> **2026-06-01 更新（`harden-host-native-conversion-service` archive 對齊；host-native conversion service 5 點誠實性/安全性 hardening）**：本 change（implementation PR #140，2026-06-01 merged，squash `c7b1dae`）已完成 **OpenSpec sync/archive**（archive folder `openspec/changes/archive/2026-06-01-harden-host-native-conversion-service/`；`openspec/specs/` 維持 **32**：`host-native-conversion-authority-service` capability ADD 4 requirement（scoped traversal-safe artifacts serving / health 反映 converter preflight / placeholder 全檔掃描 / storage sandbox root 顯式不退化 cwd）+ `streaming-ifc-usdc-conversion-authority` ADD 1 requirement（失敗 log path 結構化 `##CONV_META##` sentinel 抽取），0 MODIFIED / 0 REMOVED — additive）。本 change 來源 = 2026-06-01 風險報告 Wave 0 + Wave 2/3 conversion 同檔群（risk-triage pilot 第一個 change，收 #3/#4/#10/#11/#13）。**Verification**：pytest L2 streaming-server `test_host_native_conversion_service.py` + `test_conversion_authority_api.py` = **82 passed**（baseline 61 + 21 新 CH-1 案例）；L3 root pytest `tests` = **65 passed**（0 regression）；`openspec validate --strict` ✓。**GitNexus pre-impact**：build_app / health / _assert_publishable_outputs / Ifc2UsdcPowershellConverterAdapter / _run_powershell_conversion / adapter_from_env 6 symbol upstream 全 **LOW**，blast radius 限 conversion service Messaging module，0 cross-service impact。**Review loop**：opus 對抗式 review（5 lens；first-run 4/5 schema 失敗已改純文字重跑）抓 1 major（空字串 `storage_root=""` 繞過 cwd-fallback 契約）+ 3 minor；外部 CodeRabbit / Copilot / Codex 補抓 1 **P2 真 security**（Windows encoded backslash `%5C` 跨 job 穿越，opus security lens 被 cyber-safeguard 攔未跑成）+ health reason actionable / broad-catch / ps1 mkdir；全部 in-scope findings 已 fix（含跨 job 回歸測試）。CI：CodeRabbit + pr-review-agent passed。**Defer follow-up（使用者 2026-06-01 拍板）**：(1) placeholder 全檔 read_bytes 大模型記憶體 = demo-scope 權衡（design Key Decisions #10），production chunked scan 為效能 follow-up；(2) `_anchor` URL fallback（`edge-local://` / `file://`）用 work_dir 非 storage_root = pre-existing + 超出 #13 收斂 host_local_path 的 spec scope，IFC input sandbox 完整統一可併入後續 internal-auth change（CH-1 對 sandbox 為淨改善）。**注意**：不對 `/artifacts` 加 token-gate（屬後續 #2/#6 auth change）；不改 `/health` HTTP code（維持 200）；不改 HOOPS A3D vendor binary / `_run_ifcopenshell_openusd_fallback`；不引入新 production dependency；不破壞 callback outbox metadata-only 規約；不改 USD prim 路徑/hierarchy；不 retro-fit 既有 artifact。此 archive 未升等任何 §1.3 runtime tier 為 passed（L4 host-native 真機 conversion evidence 留待 GPU runtime）。
>
> **2026-05-28 更新（`streaming-server-ifcopenshell-semantic-sidecar-pass` archive 對齊;HOOPS happy path 後序列追加 IfcOpenShell semantic sidecar pass 解 vendor-side blocker）**：本 change（implementation PR #136,2026-05-28 merged,squash `cea66e07`;archive PR #137 sync follow-up,squash `6f41fce`）已完成 **OpenSpec sync/archive**（archive folder `openspec/changes/archive/2026-05-28-streaming-server-ifcopenshell-semantic-sidecar-pass/`；`openspec/specs/` 維持 **32**:`streaming-ifc-usdc-conversion-authority` capability ADD 1 requirement「HOOPS happy path SHALL be augmented by an IfcOpenShell semantic sidecar pass」+ 7 scenarios（HOOPS success → sidecar pass / filter Representation / enumeration reads sidecar / prefers prim CustomData / honest fallback / callback outbox metadata-only / backward compat / no retro-fit）,0 MODIFIED / 0 REMOVED — additive）。`openspec validate --specs --strict` = **32 passed / 0 failed**。本 change 解決 2026-05-26 [`c2-viewer-semantic-final-vendor-blocker.md`](../evidence/2026-05-25-fast-mvp-edge-bim-server-console/c2-viewer-semantic-final-vendor-blocker.md) 點名的 **HOOPS A3D vendor-side blocker**:HOOPS A3D library 把 IFC 視為 generic 3D 來源,`model.usdc` 10,872 prim 完全不寫 `ifc:*` / `ifcGlobalId` / `ifcType` / `ifcName` CustomData → enumeration 抽不到 → viewer Semantic ready=no。新增 `Ifc2UsdcPowershellConverterAdapter._run_ifcopenshell_semantic_sidecar`（honest non-raising:IFC missing / ifcopenshell missing / `by_type` fail / parse fail → 回 None,不寫 empty sidecar）+ static helper `_load_ifc_semantic_sidecar` / `_sidecar_entry_for_mesh_index`;`_materialize_sidecars` 在 `_adopt_converter_sidecars` 拿到 sidecars 但 `mapping_has_ifc_type` / `mapping_has_ifc_name` 全 falsy 時 SHALL 跑 sidecar pass + enumeration（CodeRabbit P0 fix:對齊 spec scenario「HOOPS success without IFC CustomData triggers sidecar pass」）;`_enumerate_usd_stage` 在 prim CustomData 空時讀 sidecar 以 enumeration order index 對齊 mesh prim 補 mapping_items + fidelity = `"usd_enumeration_with_ifc_sidecar_supplement"`;過濾 IfcProduct 有 `Representation`（CodeRabbit P0 fix:避免 IfcSite / IfcBuilding / IfcBuildingStorey / IfcSpace 等空間 product 把 mesh-prim ordinal join 全錯位）;`shape_index` 從 0 連續編號;spec.md normative body 純繁體中文,parser headers / Python identifier / IFC keywords 保留原文。**Verification**:`npx openspec validate streaming-server-ifcopenshell-semantic-sidecar-pass --strict` ✓;`npx openspec validate --specs --strict` 32 passed / 0 failed ✓;pxr-無關 helper tests 5/5 passed（含 `test_sidecar_pass_skips_ifcproduct_without_representation` + `test_sidecar_pass_returns_none_when_by_type_raises`）;root pytest `tests` 65 passed;pxr-依賴 integration tests（`test_enumeration_reads_sidecar_when_prim_custom_data_empty` / `test_enumeration_prefers_prim_custom_data_over_sidecar` / `test_materialize_runs_sidecar_pass_when_adopt_returns_semantic_falsy`）留 L4 host-native verify,對齊既有 fallback / C6 enumeration test 環境慣例（本機 sibling .venv 沒 `usd-core`,等同既有 13 個 pxr-依賴 test 在同環境一致狀態 — 0 regression）。**GitNexus pre-impact**:`_enumerate_usd_stage` upstream LOW（d=1: `_materialize_sidecars` 同 class;d=2: `convert` 同 class）;`_materialize_sidecars` upstream LOW（d=1: `convert` 同 class）;blast radius 限 `Ifc2UsdcPowershellConverterAdapter` 內,0 cross-service impact。**CodeRabbit review loop**:首版 commit `fe1eefc` 後 CodeRabbit 提 7 個 review comments（2 P0 + 3 minor + 2 P2）;P0 fix commit `3bec403` 全部處理（短路 bug 加 truthy 檢查 / by_type fail → None / 過濾 Representation / spec body 繁體中文 / tasks.md 對齊 helper 名稱 + 改 `.venv\Scripts\python.exe` venv 規範）;P2「sidecar 寫在 `/artifacts` public mount」defer with note（與既有 `element_mapping.json` / `entity_index.json` / `quality_metrics.json` 公開暴露程度一致,本 change 不增量擴大 attack surface;hardening 屬獨立 follow-up）。CodeRabbit re-review SUCCESS + pr-review-agent SUCCESS（Status warning / Risk medium / **Blockers: None** / warning 只剩 GitNexus hosted runner 已知限制）。Archive PR #137（docs-only）pr-review-agent **passed / low risk / 0 blockers / 0 warnings**。**注意**:不改 HOOPS A3D vendor library;不改 `_run_ifcopenshell_openusd_fallback`（C1 archive 已正確,且本 change 不走 fallback path）;不引入新 production dependency（`ifcopenshell` 已是 fallback path external prerequisite）;不 retro-fit 既有 archived conversion;不傳 sidecar 進 callback outbox（維持 `conversion-webhook-lifecycle` metadata-only 規約）;不改 USD prim 路徑命名或 hierarchy（會破 viewer DataChannel highlight）;不引入 async / background 寫 sidecar 流程（本 change 走序列）。**L4 真機 evidence** ⏳:host-native conversion service restart + IFC ready intake → `quality_metrics.semantic_mapping_fidelity = "usd_enumeration_with_ifc_sidecar_supplement"` + artifact dir 含 `ifc_semantic_sidecar.json` + Chrome MCP / Playwright `tri-ready-semantic = "Semantic: yes"`,evidence 寫至 `docs/evidence/streaming-server-ifcopenshell-semantic-sidecar-pass/`,於本 archive closeout 後跑（不依 archive merge 但對齊 archive sync 順序）。**roadmap §10 變化**:此 archive 並未升等任何 §1.3 runtime tier 為 passed;§10 第 9 點「持續用 `kit-mcp` / `usd-code-mcp` 做 NVIDIA extension drift check」對齊驗證:本 change 用 IfcOpenShell + `pxr.Usd` / `pxr.UsdGeom` 已知 API（usd-code-mcp 知識涵蓋範圍）,確認無 NVIDIA reference 漂移。
...
| `cross-service-structured-log-baseline` | cross-cutting | observability baseline | ✓ 跨 4 個執行單元統一 structured log：共用 JSON Schema (7 event_type)、env allow-list 脫敏、`trace_id` 跨通道傳遞、`logs/<service>/<date>/*.jsonl` 30 天 retention、coordinator viewer-log intake + health endpoint (local-dev-only no auth)、EventLog 雙 sink mirror；0 production dep；本 baseline 為 roadmap 候選 #8 `observability-audit-baseline`（Prometheus/Grafana/Loki，Phase 6 凍結）鋪 schema/檔案佈局 baseline，**不取代**；smoke evidence Kit live runtime 部分仍待後續 follow-up |
...
# 2026-06-02 author-ifc-openusd-identity-paths（identity profile evidence）
implementation PR:                     #149 merged; merge commit b612bb7
archive folder:                         openspec/changes/archive/2026-06-02-author-ifc-openusd-identity-paths/
conversion job:                         stream_conv_20260602080050_a0cc1bdc
identity profile:                       ifcopenshell_openusd_identity
mapping fidelity:                       guid_exact; coverage_ratio=1.0; unmapped_count=0; minimum_coverage_baseline_locked=false
artifact refs:                          element_mapping / entity_index / pset_index / spatial_index / bbox_index / quality_metrics / geo_reference
evidence path:                          docs/evidence/author-ifc-openusd-identity-paths/
scope boundary:                         explicit internal profile only; no Revit plugin; no default profile flip; no Docker GPU / WebRTC / dedicated multi-Kit tier upgrade
```
...
> - 2026-06-02 IFC-first identity authoring smoke：`docs/evidence/author-ifc-openusd-identity-paths/`
>
...
| `2026-05-28-streaming-server-ifcopenshell-semantic-sidecar-pass` | `streaming-ifc-usdc-conversion-authority`（ADD 1 requirement / 7 scenarios，additive） | HOOPS happy path 後序列追加 IfcOpenShell semantic sidecar pass:解決 2026-05-26 `c2-viewer-semantic-final-vendor-blocker.md` 點名的 **HOOPS A3D vendor-side blocker**(HOOPS 把 IFC 視為 generic 3D 來源,`model.usdc` 10,872 prim 完全不寫 `ifc:*` CustomData → enumeration 抽不到 → Semantic ready=no)。新 method `_run_ifcopenshell_semantic_sidecar`(honest non-raising:IFC missing / ifcopenshell missing / `by_type` fail / parse fail → 回 None)+ static helper `_load_ifc_semantic_sidecar` / `_sidecar_entry_for_mesh_index`;`_materialize_sidecars` 在 adopt 拿到 sidecars 但 `mapping_has_ifc_type` / `mapping_has_ifc_name` 全 falsy 時 SHALL 跑 sidecar pass + enumeration;`_enumerate_usd_stage` 在 prim CustomData 空時讀 sidecar 以 enumeration order 對齊 mesh prim 補 mapping_items + 推 fidelity `"usd_enumeration_with_ifc_sidecar_supplement"`;過濾 IfcProduct 有 Representation(避免 IfcSite/IfcBuilding/Storey/Space 把 mesh-prim ordinal join 全錯位);spec body 純繁體中文。Implementation PR #136 merged(squash `cea66e07`,2026-05-28),archive PR #137 sync(squash `6f41fce`,2026-05-28)。Verification:`openspec validate --specs --strict` 32 passed / 0 failed;pxr-無關 helper tests 5/5 passed;root pytest 65 passed;pxr-依賴 integration tests 留 L4 host-native verify。GitNexus pre-impact LOW(d=1 within `Ifc2UsdcPowershellConverterAdapter`)。CodeRabbit review loop:首版 commit 後 2 P0 + 3 minor + 2 P2 comments,P0 fix commit `3bec403` 全部處理(短路 bug / by_type fail / Representation filter / spec 繁體中文 / tasks 對齊 helper 名稱 + venv 規範);CodeRabbit re-review SUCCESS + pr-review-agent Blockers=None;archive PR #137 pr-review-agent passed / low risk / 0 blockers / 0 warnings。**Out of scope(defer with note)**:不改 HOOPS A3D vendor library;不改 `_run_ifcopenshell_openusd_fallback`;不引入新 production dep(`ifcopenshell` 已是 fallback path external prerequisite);不 retro-fit 既有 conversion;不傳 sidecar 進 callback outbox;sidecar 寫在 `/artifacts` public mount 與既有 mapping/quality 公開暴露程度一致,hardening 屬獨立 follow-up;async/background 寫 sidecar 留 Sidecar Pass v2 |
| `2026-06-02-author-ifc-openusd-identity-paths` | `streaming-ifc-usdc-conversion-authority`（ADD 3 requirements，additive） | IFC-first OpenUSD identity authoring explicit profile：`conversion_profile = "ifcopenshell_openusd_identity"` 直接解析 IFC、author USD stage、以 IFC `GlobalId` 建 `/World/Elements/<IfcType>/G_<encoded_guid>` stable element root，split mesh child 留在 root 下；`element_mapping.json` 只在 identity-authored IFC elements 宣稱 `mapping_fidelity = "guid_exact"`，HOOPS ordinal/sidecar matching 不得升等；artifact set 包含 entity/pset/spatial/bbox/quality/geo refs，callback outbox 維持 metadata-only，viewer 只 consume artifact refs 不成為 conversion authority。Implementation PR #149 merged（merge commit `b612bb7`，2026-06-02）；evidence `stream_conv_20260602080050_a0cc1bdc` 收於 `docs/evidence/author-ifc-openusd-identity-paths/`，`coverage_ratio=1.0`、`unmapped_count=0`、`minimum_coverage_baseline_locked=false`。**Out of scope / follow-up**：不把 identity profile 設為 default、不解 geometry fidelity/license/dependency production review、不升等 Docker GPU/WebRTC/dedicated multi-Kit、不處理 PR review 指出的 preflight/metrics/evidence-hardening 觀察 |
| `2026-06-03-governance-rule-run-service`、`2026-06-03-governance-issue-db`、`2026-06-03-a1-bcf-export`、`2026-06-03-a1-ids-import`、`2026-06-03-model-version-diff-globalid`、`2026-06-03-a2-geometry-issue-impact`、`2026-06-03-usd-federation-sublayer-sets`、`2026-06-03-a3-transform-review-room`、`2026-06-03-edge-console-shell`（2026-06-03 A1/A2/A3 + Edge Console followup 批） | `governance-rule-run-authority`、`governance-issue-tracking`、`governance-bcf-export`、`model-version-diff-authority`、`usd-federation-composition`、`usd-federation-sublayer-sets`、`edge-console-operator-frontend`（ADD，**7 新 capability**） | 落地端 governance（A1 規則 + IDS rule-run、Issue-DB 生命週期/audit、BCF 2.1 匯出）、model-version diff（IFC GlobalId 多級對齊 + geometry/issue-impact）、USD federation（sublayer 疊合 + per-member transform + Review Room handoff）與 Edge Console 操作員前端首批落地。`openspec/specs/` 32 → **39**。本批為「能力骨架/落地」change（非本輪 archive 範圍，列此供 §1.6 capability 溯源完整）；其誠實性/正確性硬化見下方 9 個對抗驗證修復 change。archive 不升等任何 §1.3 runtime tier 為 passed |
| `2026-06-03-a1-rule-engine-honesty` | `governance-rule-run-authority`（MODIFY 1「rule-run authority」requirement + ADD 1「IDS 彙總唯一可辨識/error 計數誠實」requirement） | A1 rule-engine + IDS 誠實計分修復（對抗驗證 7 findings）：governance score = passed/(passed+failed+errored)，`errored` 計入分母、視同未通過，全 error run SHALL 0.0 不得假回 100；規則真從模型萃取屬性/空間關係（排除 `get_psets()` 注入的合成 key）；跨 IFC schema 型別別名；IDS rule-run 彙總每 specification 唯一 key（不因同名互覆寫低報）、`errored` 由結果推導不寫死、prohibited specification 不得靜默通過。Implementation PR #164 merged（2026-06-03）。Verification：本輪 archive 後 `openspec validate --all --strict` 39/0；PR 於 merge 前過 pytest/openspec/對抗複驗。archive 僅代表規格已併入 + 程式已修，未取得新 runtime evidence，不標 §1.3 passed |
| `2026-06-03-issue-bcf-integrity` | `governance-issue-tracking`（MODIFY 2 + ADD 1「狀態轉換並發安全無 TOCTOU」requirement）、`governance-bcf-export`（MODIFY 1「匯出為 BCF 2.1」requirement） | Issue-DB 完整性 + BCF 匯出 provenance（對抗驗證 11 findings）：issue 以 `ifc_guid` 為主鍵綁 `model_version_id`，diff issue 綁 target 版本、缺則拒絕；無 guid → `kind=annotation`；批次建立單一交易（全有或全無）+ 同來源冪等（`skipped` 計數）；狀態轉換 DB 層交易序列化（並發只有單一贏家）；BCF 僅匯出合法 22 字元 IfcGuid（避免違反 BCF 2.1 XSD）、缺值溯源輸出 `unbound` 佔位字非 `None`、naive 時間視為 UTC。Implementation PR #165 merged（2026-06-03）。**合併備註**：「匯出為 BCF 2.1」與「可由 rule-run/diff 來源批次產生」requirement 於本輪 archive 與 `governance-adversarial-followups-2` 手動合併（見 frontmatter 2026-06-03）。Verification：archive 後 `openspec validate --all --strict` 39/0；不標 §1.3 passed |
| `2026-06-03-a2-diff-tag-typeguard` | `model-version-diff-authority`（MODIFY 1「GlobalId 多級對齊」requirement + ADD 1「同鍵簇配對穩定可重現」requirement） | A2 diff Tag 級型別護欄 + 穩定配對（對抗驗證 3 findings）：多級退階對齊每一級鍵涵蓋構件型別（`is_a()`），同 Tag 被不同 `ifc_type` 共用時 SHALL NOT 誤配（跨型別共用 Tag → removed+added，不靜默吞變更）；第三級同鍵簇以穩定次鍵（GlobalId）排序後配對，相同輸入重複執行結果可重現、不隨迭代序漂移。Implementation PR #166 merged（2026-06-03）。**合併備註**：「GlobalId 多級對齊」requirement 於本輪與 `governance-adversarial-followups-2`（Tag 唯一性護欄）手動合併。Verification：archive 後 `openspec validate --all --strict` 39/0；不標 §1.3 passed |
| `2026-06-03-a3-federation-trs-coords` | `usd-federation-composition`（MODIFY 2 requirements）、`usd-federation-sublayer-sets`（MODIFY 1「per-member transform」requirement） | A3 federation 標準 TRS + 座標一致性（對抗驗證 4 findings）：per-member transform 遵循標準 TRS（world=T·R·S，translate 為 xformOpOrder 最外層、scale 最內層），使 translate 不被 scale 連帶放大、不被 rotate 連帶旋轉；正確性以真實 pxr 計算的世界座標驗證，不僅斷言 xformOp 字面順序；federation build 前可驗證共享坐標系。Implementation PR #167 merged（2026-06-03）。**合併備註**：「per-member transform」requirement 於本輪與 `fix-federation-resetxformstack`（`!resetXformStack!` 處理）手動合併。Verification：archive 後 `openspec validate --all --strict` 39/0；不標 §1.3 passed |
| `2026-06-03-deploy-nullderef-guard` | `one-click-deploy-hybrid`（MODIFY 1「Final Summary 可診斷性」requirement） | `deploy.ps1` Final Summary / signature 讀取 null-safe（對抗驗證 2 findings）：build/probe 失敗或變數未賦值路徑下，Final Summary / signature 讀取不得因 null deref / `Set-StrictMode` 而 terminating crash，確保失敗仍可達 Print-FinalSummary 誠實回報。Implementation PR #168 merged（2026-06-03）。Verification：archive 後 `openspec validate --all --strict` 39/0；不標 §1.3 passed |
| `2026-06-03-edge-console-type-prov` | `edge-console-operator-frontend`（ADD 3 + MODIFY 1「A1 可驗證；A2/A3 骨架」requirement） | Edge Console 型別正確性 + provenance 誠實對齊（對抗驗證 5 findings，tsc 11→0）：provenance 型別接受後端權威值（含 `artifact`）；`mediaPort` 型別與串流 library 相容（`number | undefined`）；UI provenance 標示與實際落地一致（BCF 匯出為 `asbuilt`）。Implementation PR #169 merged（2026-06-03）。Verification：archive 後 `openspec validate --all --strict` 39/0；不標 §1.3 passed |
| `2026-06-03-fix-federation-resetxformstack` | `usd-federation-sublayer-sets`（MODIFY 1「per-member transform」requirement） | federation 保留 member `!resetXformStack!` 時 transform 仍套用（merge 後 reviewer P1，以 pxr 26.5 為 ground truth）：member `xformOpOrder` 以 `!resetXformStack!` 開頭時，該 token 維持 index 0、federation ops 置於 reset 之後 / member 幾何 ops 之前，使 federation translate 不因 reset token 不在第一位而被 USD 忽略（修正前 local 原點落在 `(0,0,0)`，修正後 `(100,0,0)`）。Implementation PR #170 merged（2026-06-03）。**合併備註**：本 change 與 `a3-federation-trs-coords` 改同一「per-member transform」requirement，本輪 archive 手動合併兩者意圖（標準 TRS + reset 處理，最終 4 scenarios）。Verification：archive 後 `openspec validate --all --strict` 39/0；不標 §1.3 passed |
| `2026-06-03-frontend-coordinator-env` | `edge-console-operator-frontend`（ADD 1「console A1/A2/A3 client 用一致 coordinator base env 名」requirement） | console governanceClient 用正規 `VITE_COORDINATOR_API_BASE`（對抗複驗 HIGH）：console 的 A1/A2/A3 client SHALL 用與全站／部署一致的 coordinator base env 名，避免自訂 env 名導致部署接線錯誤。Implementation PR #171 merged（2026-06-03）。Verification：archive 後 `openspec validate --all --strict` 39/0；不標 §1.3 passed |
| `2026-06-03-governance-adversarial-followups-2` | `governance-rule-run-authority`、`governance-issue-tracking`、`governance-bcf-export`、`model-version-diff-authority`（各 MODIFY 1 requirement） | 最終對抗複驗第二層 IDS／diff／issues／BCF 誠實與正確性（7 findings）：F1 IDS 重用 specs 物件跨 model 不殘留假通過（每次驗證進入點重置 ifctester requirement facet 殘留通過集合）；F2 required spec 零適用構件誠實 fail 非 100% pass；F3 同型別多個相同 Tag 不幻影配對（`(ifc_type, Tag)` 各側恰 1 個才配對）；F4 from-rule-run 版本綁定刻意不對稱（rule-run best-effort 可空 / diff 缺 target_model_version_id 回 422）；F5 BCF GPLv3 依賴敘述誠實校正（匯出模組執行期不 import bcf-client，惟 ifctester transitive 安裝 GPLv3 bcf-client）。Implementation PR #172 merged（2026-06-03）。**合併備註**：本 change 與 `a1-rule-engine-honesty`/`issue-bcf-integrity`/`a2-diff-tag-typeguard` 改重疊 capability，本輪 archive 手動合併各 requirement 意圖（細節見 frontmatter 2026-06-03）。Verification：archive 後 `openspec validate --all --strict` 39/0；不標 §1.3 passed |
...
| HTML 檢視版（ignored 衍生檔） | 如需人類檢視，可用文件/規劃相關 skill 由本 Markdown on-demand 產生同名 HTML | 不得讓 HTML 成為 source of truth；不得把 `docs/plans/*.html` 加入 PR；不得只改 HTML 而不更新 Markdown |
...
6. 如需 HTML 檢視版，可本機 on-demand 重新產生，並確認 `git ls-files docs/plans/*.html` 仍為空。
```
...
- `docs/plans/*.html` 未被提交；如需檢視版，已確認可由同名 Markdown on-demand 產生。
- 若 roadmap 未更新，archive 只能視為規格檔案已搬移，不能視為專案執行規劃已收斂。
- 若需要 HTML 檢視版，只能作 ignored 衍生檔；archive closeout 不得因提交 HTML 破壞 `documentation-source-of-truth`。
```
...
>
> **2026-06-02 archive 補充**：`author-ifc-openusd-identity-paths` 已把 IFC-first OpenUSD identity authoring explicit profile 移入 archived specs；它補的是 stable IFC identity / `guid_exact` mapping / ai-bim-geo sidecar refs，不取代既有 HOOPS/fallback profile，也不把 identity route 設為 production default。
...
| `identity_authoring_profile` | `passed(explicit profile smoke)`：`stream_conv_20260602080050_a0cc1bdc` 以 `ifcopenshell_openusd_identity` 產生 stable element root、`guid_exact` mapping 與 ai-bim-geo sidecar refs | 若要改成 default 或 production baseline，需另開 change 補 preflight/metrics/evidence hardening 與 license/runtime review |
...
Completed identity-authoring profile:
  author-ifc-openusd-identity-paths
    → explicit `ifcopenshell_openusd_identity` profile 已可用 IFC `GlobalId` author stable OpenUSD identity path、`guid_exact` mapping 與 ai-bim-geo sidecar refs；後續要 production/default 化再另開 hardening change。
...
2. **B-scheme real streaming conversion + single Kit/WebRTC visual proof 已完成（current single-job baseline）**：
   ...
   - IFC-first identity authoring explicit profile 已歸檔：`openspec/changes/archive/2026-06-02-author-ifc-openusd-identity-paths/`；evidence `docs/evidence/author-ifc-openusd-identity-paths/` 顯示 `ifcopenshell_openusd_identity` 可產生 stable element root、`guid_exact` mapping 與 ai-bim-geo sidecar refs。
...
9. **保留本文件與 next burn-down 作為下一輪對照基準**：
   ...
   - 每次 OpenSpec sync / archive 後，依 §1.6 同步 `§1.2` / `§1.4` / `§5` / `§6` / `§10`；HTML 檢視版只作 ignored on-demand 衍生檔，不提交。
