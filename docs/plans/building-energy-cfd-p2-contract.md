...
| GET | `/api/cfd/options` | S8：透傳 `cfd-options/v1`（以 zod 驗證）；不需 operator scope |
| POST | `/api/cfd/estimates` | S8：body 以 `cfd-estimate-request/v1` 驗證後透傳，回 `cfd-estimate/v1`；不需 operator scope、不寫 ledger |
