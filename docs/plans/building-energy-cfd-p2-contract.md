...
| 風環境面板以模型為主體（owner 2026-09-22 裁決方向 1） | 現況：面板從 session 的 stream-config 取模型，無 session 什麼都不列；只列該 session 綁的模型的 run；ledger 無 session／參數摘要／佇列位置；streaming 單 worker FIFO。方向 1：面板加模型選擇器（`/api/conversion/records`）、無 session 亦可瀏覽／送出，session 只在「顯示疊圖」時需要；ledger 加 optional `origin{session_id, submitted_at, params 摘要}` 與 `queue_position`；加跨模型「風場計算總覽」。契約為 additive optional，落點 coordinator ledger＋面板，不新增儲存層 |
