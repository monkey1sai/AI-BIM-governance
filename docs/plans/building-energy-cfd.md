...
- **進度**：owner 2026-09-23 選方向 1，先做 A 階段，並同意部署。A 階段拆成兩個 PR：A1（#911，選項端點、預估、算力上限、契約）與 A2（#912，面板計算設定區）已合併並部署（`e2231f0`）。2026-09-23 用 Chrome MCP 完成真站驗證，估算格數誤差 +0.42%、耗時誤差 +9.6%，證據見 `docs/evidence/cfd-s8-2026-09-23/`。owner 同意 A 收尾後，B 階段（引擎新參數與 AIJ 驗證）先提契約草案，核准後才實作；C 階段（驗證通過的預設組）排在 B 之後。細節見 P2 契約 S8 列。
