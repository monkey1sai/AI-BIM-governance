...
`design_reference.snapshot`：default screens=13；goldens=26；read-only authoring snapshot=`design-doc.html`。`branch_protection.snapshot`：Require PR=true；`required_approving_review_count=0`；`require_code_owner_reviews=false`；`dismiss_stale_reviews=true`；`require_last_push_approval=false`；`enforce_admins=true`；strict required contexts=11；review/ruleset bypass=none；force push=false；branch deletion=false。`review_trust.status=not_established`；OpenSpec task 2.7=未完成。`remote_main.design-semantic-visual.producer=absent`；required context=absent。`semantic_contract.status=state_variants_reference_missing`；`implemented_case_ids=[]`；functional/runtime machine producer/validator=absent。viewer lockfile=untracked；runner pin=`windows-2025` label only；resolved dependency tree/runner image/font fingerprints=un-pinned。frontend product design job=fail closed；`full_completion_allowed=false`；production 99% alignment=`not observed`；`product_pass.status=not_established`。
