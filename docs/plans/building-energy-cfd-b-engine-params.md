...
| 加細盒 | `refinement_box_for` | 建物表面加細 2 級；只有一個加細盒，加細 1 級。bbox 模式是 bbox 外擴上游、側向、上方各 1H，下游 2H。等向模式（service 預設）以外殼頂點重心為圓心、最遠頂點距離為半徑，在外接圓外擴同樣的 1H／2H，所以離 bbox 更遠。以 S5b-2 的外殼、風向 0° 為例，現行定義的盒子離 bbox 上游約 7.2H，已超出 5H 的入口，下游約 4.6H；這些距離會隨風向改變。S5b-2 的網格是在 `29e4eb9` 改成這個定義之前跑的，當時約 2.6H、3.6H（第 7 節實跑更正）。AIJ 用 bbox 模式 |
...
- 等級：等於加細盒等級（放粗時同樣加 n）。
- 帶高：snappyHexMesh 依格心判定一個格子要不要加細，加細到該區的等級就停。所以帶頂要避開帶還要加細的各級格子的格心，也就是從最粗地面格那一級到帶等級減 1；帶等級本身的格心不影響結果。
  - 取 `(m + ¼) · h`，h 用帶等級減 1 那一級的格距（帶格的 2 倍），就不會落在任何較粗等級的格心上。
  - 以 V2 為例（背景格 1.5 m、放粗 1 層）：放粗後背景格是 3 m，1.5 m 正好是它的格心，所以不能直接用 0.1H（1.5 m）。帶格是 0.75 m，h 取 1.5 m，可用 1.875 m（0.125H）或 3.375 m（0.225H）。
  - 以上依 snappy 的格心判定推論。
- 厚度：帶內至少要有 2 層該等級的格子；帶高也要超過沿途最粗地面格的半格高，否則入口到外殼之間的地面完全不會加細。最粗地面格就是放粗後的背景格；若某層外殼已涵蓋整條帶在計算域內的部分，而且高過背景地面格的格心（例如上游很短），則是這種外殼裡最內層那一級。
- 上游距離：加細盒碰到入口時，上游沒有地面可加細。
- 這些條件要等 H 與格距確定才能判斷，所以在 `build_case` 檢查，連同「帶高」一點的格心條件；不成立時讓 case 寫入失敗並說明。檢查用的是寫進 dict 的 6 位有效數字值，因為 blockMesh 與 snappyHexMesh 讀到的就是這個值。
...
**結果（2026-09-24，引擎 `6a18fbd`）：** 證據與完整表格見 `docs/evidence/cfd-b-s7-2026-09-24/README.md`。

- AIJ V1、V2 通過全部五項門檻：格數約為 E4 的 36%、耗時約 40%，hit rate 0.467、0.400，FAC2 0.850、0.858。
- 實案放粗 1、2 層通過兩項準確度門檻（峰值 −1.20%、−1.13%，面積平均 +0.06%、+0.18%），但格數 55.7%、50.6%，耗時 55.8%、53.6%，未達「至少減半」。
- 原因是這棟建物的等向加細盒很大（0° 時上游端超出入口），放粗只能減少盒外的背景格。以證據的數字推算，0° 時放粗最多只能降到基準的約 49%。
- V2 的地面帶取 0.2H（3 m），不是第 3 節舉例的 (m + ¼)·h。3 m 的帶頂落在格面上，離它還要加細的兩級格心都是半格，同樣避開格心；(m + ¼)·h 只是充分條件。
- 五個執行實際耗時合計約 3.8 小時。
