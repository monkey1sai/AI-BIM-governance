...
| 加細盒 | `refinement_box_for` | 建物表面加細 2 級；只有一個加細盒，加細 1 級。bbox 模式是 bbox 外擴上游、側向、上方各 1H，下游 2H。等向模式（service 預設）以外殼頂點重心為圓心、最遠頂點距離為半徑，在外接圓外擴同樣的 1H／2H，所以離 bbox 更遠。以 S5b-2 的外殼為例，現行定義的盒子離 bbox 上游約 7.2H，已超出 5H 的入口，下游約 4.6H；S5b-2 的網格是在 `29e4eb9` 改成這個定義之前跑的，當時約 2.6H、3.6H（第 7 節實跑更正）。AIJ 用 bbox 模式 |
...
- 原因是這棟建物的等向加細盒很大（上游端超出入口），放粗只能減少盒外的背景格。
- 五個執行實際耗時合計約 3.8 小時。
