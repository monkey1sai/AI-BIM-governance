...
> **2026-07-29 design gate 現況（唯讀查證，subject `13033cb`）：** main 的 `design-semantic-visual` 為 **FAILURE** 且會持續紅到有人處置。唯一失敗項 `workspace.a4.default`（diff ratio `0.2794` / `0.3186` > 上限 `0.01`），其餘 12 screens PASS、`semantic_parity = 1`。成因**不是**樣式回歸，而是 `#a4` 的 route IA 遷移：`EdgeConsole.tsx` 的 `UNIFIED_WS_KEYS` 已移除 `a4`，`#a4` 改渲染 `A4SemanticSearchPage`（A4 dock tab 與 `A4Dock` 元件仍在，面板被掏空為導流卡；不可達的是 golden 描繪的**已填充**版本）。另查證：pinned origin `C:\Repos\design\desigin-system` 對 `manifest.source.files` **23/23 hash MATCH**（rebaseline 對 `source.files` 為 no-op，該面不需設計側核准）。完整證據見 `openspec/changes/isolated-branch-stack-browser-e2e/proposal.md`「相鄰既有缺口」D-1～D-13。**歸屬**：D-1/D-2 → `a4-console-convergence`；D-5 → `migrate-console-to-hifi-design` task 6.4（human owner only）；D-8 已具備 canon MUST 與同 repo 參考實作、修法 pixel-neutral，**只缺 owner 指派**；其餘 D-3/D-4/D-6/D-7/D-9/D-10/D-11/D-12/D-13 → **待使用者裁決**，本輪不由任何 change 代為指派 owner。

> **2026-07-29 三層交叉對抗驗證結果：** 依使用者指令對上述缺口衍生的設計問題執行三層驗證（L1 提案 → L2 三視角 refute-by-default → L3 仲裁，基準 `13033cb`，全程唯讀）。**L1 多數裁決被推翻並撤回**，含「A4 改 code 回 dock」「擴充 capture 腳本加第三旗標」「spectator 預算固定為 1」「A5–A10 dashboard 殼可先做」。關鍵推翻證據：(a) 恢復 dock 與 `a4-console-convergence` 自身 tasks 3.3／3.4 直接對撞——#427 落地的正是該 change 的 task；(b) golden 由 `authority.authoring_origin` 擷取即 canon 投影，其畫面內含 `不符合 5 · 符合 7` 等 fixture 數字，故「production 改誠實但 canon 不動」在 `max_diff_pixel_ratio = 0.01` 下**結構性不可能通過**；(c) `KIT_SPECTATOR_COUNT` 的 active canon 預設為 **0**（`documentation-source-of-truth/spec.md:208-209`），部署層的 5 本身即為 canon 衝突。驗證另新增 D-6～D-13 八項缺口與 U-1～U-12 待裁決清單，全部記於同一份 proposal。收斂輪再驗後又撤回多項 owner 指派（D-3／D-6／D-10）與一項產品裁決（A5–A10 的 3D 路徑選型 → U-10），並修正兩處事實錯誤：`design-semantic-visual` **不在** branch protection 的 required contexts 內（初版誤稱 branch-protected 會灌高緊急度）；本 PR 的該 job 實際為 **skipped** 而非 failure。依 proposal 分析，U-1（canon 的 fixture 數字與 R3 誠實鐵律哪一邊讓步）是其餘多數項的上游依賴；**排序仍由你決定**。
...
| 2026-07-29 | design gate 唯讀查證＋三層交叉對抗驗證：main `design-semantic-visual`（**advisory job，非 required context**）紅燈成因＝`#a4` route IA 遷移，非樣式回歸。**rebaseline 關不掉此紅燈**——capture 腳本無 per-screen scope 且 baseline 自 canon 擷取，canon 未變則重截等價。缺口 D-1～D-13 與待裁決清單 U-1～U-12 記入 `isolated-branch-stack-browser-e2e` proposal；多數 owner 指派已撤回，退回使用者裁決。 |
