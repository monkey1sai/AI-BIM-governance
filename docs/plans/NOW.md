...
> **2026-07-30 isolated stack progress：** PR #431 verified evidence subject `cb362130d0d51b1d6d1b7e23ccf87343b14286ce`（P1 SafeHandle process-handle pinning、P5 publication head binding）／P4 run `p4-final-20260730-115300`／已對齊 27/33 tasks。`npm run verify`（78 files／938 tests）與 isolated launcher tests 通過；6 個 require-real Chromium cases 在 manifest status/head gate、isolated health 後因現有 A4 沒有 downloaded IFC-ready job 而 fail closed，未產生成功證據或 observed runtime ID。未勾項為：2.5（缺少 `Invoke-ScriptAnalyzer` 靜態 gate）、4.3（base-identical governance test 的 GitNexus integrity mismatch）、5.2／5.3（A4 fixture／成功 evidence 缺口）、6.5／6.6（待本次 PR body 與最新 CI）。6.3 依 GitNexus UNKNOWN/unavailable gate 記錄 Sol sign-off；不宣稱 GitNexus pass。pinned reconciler 的全域無關 `cross-service-structured-log-baseline` lifecycle mismatch 仍須獨立揭露。
