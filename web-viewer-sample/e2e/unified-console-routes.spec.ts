...
test.describe("B 操作台：:8004/ui UnifiedConsole 路由 + 保留 operator-tool 路由", () => {
  test("/ui 掛 UnifiedConsole home；保留 #/kit、#/demo-control、#/review 可達；#/runtime＝新 Ops", async ({ page }) => {
    ...
    await page.goto(`${CONSOLE}/ui#/runtime`);
  ...
  test("UnifiedConsole 側欄點擊切換 A1→A2（hash 更新 + dock tab 高亮，零依賴 hash 路由）", async ({ page }) => {
    ...
    await expect(page.getByTestId("ws-tab-a2")).toHaveAttribute("aria-selected", "true");
    await expect(page.getByTestId("ws-tab-a1")).toHaveAttribute("aria-selected", "false");
    await expect(page.getByRole("heading", { name: "模型版本差異與責任追蹤 · A2" })).toBeVisible({ timeout: 15_000 });
