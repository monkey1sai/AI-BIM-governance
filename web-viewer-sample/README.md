...
### BIM Review Demo Panel

For the AI-BIM governance MVP, the local build shows a `Demo Controls` panel by default. It is controlled by:

```txt
VITE_SHOW_DEMO_PANEL=true
```

The panel can create or load a coordinator review session, load review-bootstrap data, reconnect Socket.IO, send `openStageRequest`, `loadingStateQuery`, `getChildrenRequest`, `highlightPrimsRequest`, `focusPrimRequest`, and `clearHighlightRequest`, emit coordinator `highlightRequest`, and create a demo annotation.
