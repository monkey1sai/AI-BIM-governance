...
export default function DemoControlPanel(props: DemoControlPanelProps) {
    ...
    const interactionLabCards: InteractionLabCard[] = [
        ...
        },
        {
            ...
            status: "RETIRED",
            statusKind: "idle",
            description: "2026-05-21 起 Socket.IO highlightRequest / selection 協作 handler 已自 product runtime 移除；僅 join/leave/heartbeat/presence。",
            effect: `現況：/review 不處理 highlightRequest。最近 socket 事件「${latestSocketEvent}」（可能為 presence 或 client 本地 log）。`,
            actionLabel: "已退役 · 不可用",
            action: () => undefined,
            disabled: true,
            gap: "正式 3D 高亮請用 DataChannel highlightPrimsRequest（上一步 COL≠此）或 console client highlight。",
        },
        {
            num: "REC",
            title: "建立標註並準備回寫（已退役）",
            ...
            status: "RETIRED",
            statusKind: "idle",
            description: "annotationCreate Socket.IO 協作路徑已退役；Issue 請走 governance Issue/BCF（A1/A4 console）。",
            effect: "不會再出現 live annotationCreate ack。",
            actionLabel: "已退役 · 不可用",
            action: () => undefined,
            disabled: true,
            gap: "改用 Edge Console #issues 或 A1/A4 批次建 Issue。",
        },
        {
            num: "GAP",
