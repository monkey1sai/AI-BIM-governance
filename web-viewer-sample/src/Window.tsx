...
export default class App extends React.Component<AppProps, AppState> {
    ...
    private commandChannel = createViewerCommandKitSide({
        snapshot: () => this._sectionSnapshot(),
        requestId: () => createRuntimeRequestId(),
        send: message => this._sendStreamMessage(message),
        correlate: (eventType, payload) =>
            this.runtimeCommandTracker._correlateRuntimeCommandEvent(eventType, payload).disposition === "matched",
        claimTerminal: (requestId, command, outcome) => {
            if (this.runtimeCommandTracker.hasContext(requestId)) {
                this.runtimeCommandTracker._claimRuntimeCommandTerminal(requestId, command, outcome);
            }
        },
        post: message => {
            if (message.type === "measurement_state" && this.componentMounted) {
                this.setState({ measurement: this.commandChannel.measurement.state });
            }
            this._postToParent({ ...message });
        },
    });
    private _cancelMeasurementKey = (event: KeyboardEvent): void => {
        if (this.commandChannel.measurement.capturesInput) {
            event.preventDefault(); event.stopImmediatePropagation();
            if (event.key === "Escape" && event.type === "keydown") this.commandChannel.measurement.control("cancel");
        }
    };
    ...
    private _sendStreamMessage(
        ...
        void AppStream.sendMessage(outgoing)
            ...
            .catch(() => {
                ...
                if (!isRuntimeMutator(outgoing.event_type) && isRecord(outgoing.payload)) {
                    this.commandChannel.failTransport(outgoing.event_type, getPayloadString(outgoing.payload, "request_id"));
                }
                if (runtimeRequestId) {
                    if (!this.runtimeCommandTracker._claimRuntimeCommandTerminal(runtimeRequestId, outgoing.event_type, "error")) return;
                    this.commandChannel.failTransport(outgoing.event_type, runtimeRequestId);
    ...
    private _onViewerCredentialsChanged(next: ViewerCredentials): void {
        ...
        if (previous?.epoch !== next.epoch) this.commandChannel.sync();
    }
    ...
    */
    private _handleCustomEvent(
        ...
        if (event.event_type === "commandRejected") {
            ...
            if (parsed.request_id) {
                this.commandChannel.rejectCommand(parsed.rejected_event_type, parsed.request_id);
                this._finishA4HandoffCommand(
                    parsed.request_id,
                    "rejected",
        ...
        return (
            ...
            >
                ...
                <AppStream
                    ...
                    />}
                {this.commandChannel.measurement.capturesInput && <div data-testid="measurement-pick-surface"
