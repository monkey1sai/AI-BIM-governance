...
// 既有導覽 + FlowBar + 真實共享狀態；不顯示不可操作的 Agent 或情境預覽。
...
// 五步導航沿用既有頁面；state 僅是工程 metadata，不宣稱實際執行或驗證結果。
    ...
    <div className="ec-flow">
      {FLOW.map((f, i) => (
        <span key={f.tech} style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <button
            className={`ec-flow-step ${active === f.page ? "active" : ""}`}
            data-prov={f.state}
            onClick={() => go(f.page)}
            title={register === "biz" ? f.biz : f.tech}
          >
  ...
  return (
    ...
    <div className="ec-root ec-agent-collapsed">
      ...
      <nav className="ec-nav">
        {NAV_GROUPS.map((group) => (
          <div key={group.key} className="ec-nav-group" data-group={group.key}>
            ...
            {PAGES.filter((p) => p.group === group.key).map((p) => (
              <button key={p.key} className={page === p.key ? "active" : ""} data-plane={p.plane} data-badge={p.badge} title={navText(p.key, p.label)} onClick={() => go(p.key)}>
        ...
        <div className="ec-nav-boundary">{t("模型檢視與審查紀錄", "Model viewing and review records")}</div>
      </nav>
      ...
      <footer className="ec-foot">
        <span>{t("模型檢視與審查紀錄", "Model viewing and review records")}</span>
        <span style={{ flex: 1 }} />
        <span>{t("資料狀態以各頁顯示為準", "See each page for its current data status")}</span>
      </footer>
    </div>
    </SharedStatusProvider>
  );
