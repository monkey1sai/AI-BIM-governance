...
it("keeps controls outside the viewport and starts with no assumed effective state", () => {
  act(() => root.render(<SectionPlaneControls ready={false} state={{ status: "idle" }} commands={noCommands} />));
...
it("sends explicit coordinates and prevents pending actions", () => {
  const send = vi.fn();
  const commands = fakeViewerCommandPort({ section_plane: send });
  act(() => root.render(<SectionPlaneControls ready state={{ status: "idle" }} commands={commands} />));
  ...
  act(() => root.render(<SectionPlaneControls ready state={{ status: "pending" }} commands={commands} />));
...
it("allows off after an invalid unsubmitted draft and sends a safe off payload", () => {
  const send = vi.fn();
  act(() => root.render(<SectionPlaneControls ready state={{ status: "applied", effective: { enabled: true, axis: "x", direction: -1, position: 15 } }} commands={fakeViewerCommandPort({ section_plane: send })} />));
...
it("separates effective result from editable draft and shows invalidation", () => {
  act(() => root.render(<SectionPlaneControls ready state={{ status: "applied", effective: { enabled: true, axis: "x", direction: -1, position: 15 } }} commands={noCommands} />));
  expect(box.textContent).toContain("已回覆設定：X");
  expect(box.textContent).toContain("15");
  act(() => root.render(<SectionPlaneControls ready state={{ status: "unconfirmed" }} commands={noCommands} />));
...
it.each(["rejected", "transport", "timeout", "readback"] as const)("renders a human reason for %s", reason => {
  act(() => root.render(<SectionPlaneControls ready state={{ status: "error", reason }} commands={noCommands} />));
