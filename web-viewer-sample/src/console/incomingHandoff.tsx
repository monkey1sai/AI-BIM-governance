...
export type HandoffVerifyStatus = "none" | "verified" | "not_found" | "indeterminate";

export function useIncomingHandoff(
  selfAxis: AxisKey,
  // verify 回三態：true=命中權威資料（verified）；false=誠實查無（not_found）；"indeterminate"=目前無法判定
  // （權威資料被查詢窗上限截斷、或尚未載入）→ 誠實顯「未明」，不誤報 not_found（quality Important #4；§5.4）。
  verify: (h: CrossAxisHandoff) => boolean | "indeterminate",
  ...
  const status: HandoffVerifyStatus =
    result === true || latched ? "verified" : result === "indeterminate" ? "indeterminate" : "not_found";
...
export function IncomingHandoffBanner({ testId, handoff, status }: { testId: string; handoff: CrossAxisHandoff | null; status: HandoffVerifyStatus }) {
  ...
  // indeterminate（權威資料未完整載入或超出查詢窗）是中性態、非警示：不掛 ec-warn-note，也不宣稱已重驗或查無。
  const text =
    status === "verified"
