...
function rt(activeCount: number): RuntimeStatus {
  return {
    service: { status: "ok", name: "bim-review-coordinator", uptime_seconds: 1, generated_at: "2026-07-03T00:00:00Z" },
    ...
    sessions: { count: 1, active_count: activeCount, participant_count: 0, items: [
      fx.runtimeSessionSummary({ session_id: "review_session_a", status: "active", project_id: "270", model_version_id: "v1", participant_count: 2, expected_stage_url: null, conversion_status: "ready", kit_instance_ids: [], created_at: "", updated_at: "" }),
    ] },
...
describe("SharedStatusProvider", () => {
  ...
  it("polls runtimeStatus once per cycle and maps sessions + null GPU + designed-null stage_matched", async () => {
    ...
    vi.spyOn(coordinatorClient, "getConversionRecords").mockResolvedValue({ count: 2, items: [
      fx.conversionRecord({ idempotency_key: "k1", project_id: "270", project_display_name: "270", category: "c", external_model_version_id: "v", conversion_job_id: null, status: "queued", usdc_key: null, coverage_report: null, object_key: null, detected_at: "", updated_at: "" }),
      fx.conversionRecord({ idempotency_key: "k2", project_id: "270", project_display_name: "270", category: "c", external_model_version_id: "v", conversion_job_id: null, status: "ready", usdc_key: null, coverage_report: null, object_key: null, detected_at: "", updated_at: "" }),
    ] });
