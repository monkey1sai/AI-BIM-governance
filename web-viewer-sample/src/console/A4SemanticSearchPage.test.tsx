...
describe("A4SemanticSearchPage", () => {
  ...
  ])("disables %s when the selected session becomes inactive", async (_label, response, testId) => {
    vi.mocked(coordinatorClient.runtimeStatus)
      .mockResolvedValueOnce(ACTIVE_SESSIONS)
      .mockResolvedValueOnce({
        sessions: { items: [{ session_id: "review_session_a", status: "active" }] },
      } as never);
    vi.spyOn(governanceClient, "searchModelForSession").mockResolvedValue(searchResponse(response));

    await mount();
    await act(async () => {
      (container.querySelector('[data-testid="a4-run"]') as HTMLButtonElement).click();
    });
    const action = () => container.querySelector(`[data-testid="${testId}"]`) as HTMLButtonElement;
