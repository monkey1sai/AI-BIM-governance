...
- Hold one environment-plus-service controller lease from merge preparation through terminal attestation. A repeated identical active request returns typed non-terminal `idempotent_active` ownership with no new transition or terminal append and must stop before deployment; another controller or a consumed replay key is `HELD`. The controller consumes retry evidence and appends every accepted attempt state transition to the previous ledger digest.
...
- Retry is bounded, failure-class deduplicated, linked to its prior terminal attempt, and event-driven. One failure class gets at most one retry, unchanged evidence cannot trigger re-evaluation, and every accepted retry attempt ends with a terminal record.
