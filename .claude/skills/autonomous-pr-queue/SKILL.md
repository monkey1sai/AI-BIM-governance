...
- Retry is bounded to one total same-commit attempt and only the closed `network_transient` class. The candidate must link the original root `FAILED/MERGED_NOT_DELIVERED` terminal record for the same delivery, merge, and target. An owner policy broker must externally verify an expiring signed classification binding the parent digest, event evidence, artifact, target fingerprint, command, and policy digest, then atomically compare-and-consume the retry budget keyed by delivery ID, trusted merge SHA, and root-attempt digest. Only authoritative prior count zero plus a successful consume may continue. Caller-supplied history, a new signature, or a new authorization ID cannot replenish the budget. A retry of a retry, a renamed class, a delivered or held parent, missing authority, changed binding, or consumed budget is `HELD`; every accepted retry attempt ends with a terminal record.
