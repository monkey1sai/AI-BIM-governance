...
Any mutation need, private-input need, unknown or non-durable conversion ledger, missing command lineage, empty lineage, malformed IFC observation, any failed or nonterminal in-scope lineage, intake/linkage/metadata-binding failure, failed verified idempotency comparison, proven unexpected RVT ledger entry, non-exhaustive result, privacy-redaction failure, or cross-run linkage retained in tracked files is `HELD`, not a pass.
