...
- If `CONVERSION_LEDGER_STORE_PATH` is unknown or not proven durable, the whole acceptance is `HELD`. A snapshot is evidence, not a restore mechanism. Since issue #531 landed, the canonical wiring is compose-declared rather than env-file-supplied: both coordinator stores are tracked literals in `compose.runtime-manager.yml` under the mounted runtime storage volume (`/workspace/storage/coordinator/`), so this variable never appears in the deploy report's redacted effective-env snapshot. The accepted read-only proof has two parts that must both hold: (1) the deployed revision — resolved from the governed deploy report's recorded deploy tag or checked-out sha (reports live under `artifacts/deploy-reports/canonical-linux/`), never from the operator's working tree — declares `CONVERSION_LEDGER_STORE_PATH` in tracked `compose.runtime-manager.yml` as a literal container path under the mounted runtime storage volume, and (2) the currently running coordinator's own startup `env_snapshot` structured-log line (emitted once per process at bootstrap, redacted to type and length) showing the same variable present with a value length equal to that tracked literal's length, from a container whose creation time falls inside that same deploy report's recorded `execution_window` (persisted by the deploy helper since the same change) — not merely after it. The window requirement is what defeats length spoofing: the runtime log alone cannot prove value equality (it exposes only type and length), but substituting a same-length different path requires a recreate, and any recreate outside the governed deploy window moves the container creation time out of the window. A container created outside the window — whatever its environment — means durability is unproven until a fresh governed deployment produces a new report; that, a missing part, or a length disagreement is `HELD`. Runtime APIs deliberately do not expose store paths. A deploy report without a recorded `execution_window` predates #531 and cannot support this proof; that is `HELD` until a fresh governed deployment produces a new report.
