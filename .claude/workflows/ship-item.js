...
8. 查 reviewer P1/P2 發現（三處來源，全部 --paginate）：只偵測 P1/P2 等級關鍵字（P1、P2、Blocker、Critical、High、CHANGES_REQUESTED；Blocker/Critical 視同 P1，High 視同 P2），避免把 nit / low / medium / style-only 建議升級成自動修復輸入；任一處有未解除 P1/P2 finding 都 hold。HEAD=$(gh pr view <n> --json headRefOid --jq .headRefOid)。(a) inline diff comment：gh api --paginate repos/${REPO}/pulls/<n>/comments | jq -s "add | map(select(.commit_id|startswith(\\"\${HEAD:0:9}\\")))"（用 commit_id「現所在 commit」非 original_commit_id「首次 commit」，後者會漏掉留在當前 head 的新 comment；不用 group_by）。(b) PR-level review（summary / CHANGES_REQUESTED，整體 verdict 常落這）：gh api --paginate repos/${REPO}/pulls/<n>/reviews。(c) PR 對話串 issue comment（pr-review-agent summary 的 Blockers 走 issue endpoint，不在 /pulls/comments 內）：gh api --paginate repos/${REPO}/issues/<n>/comments。對每個 P1/P2 finding，SHALL 建立穩定 key：source + file/path + line/anchor + normalized finding text，作為 carry-forward 與同一處不重複 autofix 的依據。
9. 跨 push carry-forward 與 autofix gate：不可只看「當前 head 是否還有新 comment」就放行。reviewer 在舊 head 提出的 P1/P2，若 push 新 head 但並未真正修復（reviewer 未重貼確認、或只是 commit_id 被推離當前 head），仍視為未解除。SHALL 自行維護一份「已知未解除 P1/P2 finding」清單，跨每次 push 沿用、逐項判斷是否確已修復，不可清空重來；comment 因 commit_id 移出當前 head 而被篩掉，不可據此當已解決。P1/P2 finding 進入 autofix 前 MUST 啟動交叉對抗驗證：builder 提出最小修法與驗證；verifier 反查 source of truth、blast radius、是否已修過同一 key、是否可能是假陽性或產品決策；coordinator 裁定 autofix / hold for user / reject as false positive。同一 finding key 在同一 PR 生命週期內最多只允許一次 autofix 嘗試；若同一處再被 reviewer 重貼或 autofix 後仍失敗，停止第二次自動修補並 hold 回報。
...
11. 有新 P1/P2 finding（或 carry-forward 清單仍有未解項）→ 先做交叉對抗驗證 → 若裁定 autofix，做一次最小修補 → push → 對每一次 push 各自重跑 step 6-10 的 buffer cycle；同一 finding key 不得第二次自動修補，SHALL NOT 只看 check 狀態就 merge。
