...
- **Invoke via:** an explicit security-audit request, or a security-sensitive step in the repo's ship workflow.
- **Do not invoke from another persona.** If `code-reviewer` flags something that warrants a deeper security pass, the coordinator or user initiates that pass — not the reviewer. See [agents/README.md](README.md).
