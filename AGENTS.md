...
- 已授權但限縮：(a) 明確啟動的 `spec-to-done` 可在目前 spec PR 已 merge、commit 可由 freshly fetched `origin/main` 取得後，於測試部署區真實驗證前執行 ownership-gated preflight；無參數預設只偵測，只有明確傳入 `-StopOwnedRuntime -DeploymentRoot 'D:\Users\deploy\AI-bim-geo'`，且 listener 符合 per-port service role、deployment pidfile ancestor 與精確 launcher entrypoint、creation identity 經雙快照與 stop 前重驗一致，才可用 exact process handle 停止。pidfile 僅供 lineage 佐證，caller 不得覆寫 deployment topology；必須記錄 port / PID / process name / ownership kind，再執行同一條 `-Build`。(b) 既有一般 Phase 3 重試能力保留，但所有自動停止也 MUST 走同一 helper 與相同閘門，再重跑同一條 `-Build`；helper 無法證明 ownership 時必須 HELD，只有使用者逐次確認明確 PID 與證據後才可人工例外。不得改用 `-Force` / `-DryRun`、驗證未 merge branch，或停止無關 process。
