...
describe("T5 cloud callback outbox（metadata-only / retry / dead-letter）", () => {
  it("ready 結果→組 conversion_result_ready（僅 metadata refs）入 outbox，conversion 與 callback 狀態分離", async () => {
    ...
    const res = await request(app.app).post("/api/internal/conversion-result").send({
  ...
  it("cloud 不可達 → outbox 保留重試，retry 耗盡 → dead_letter（不靜默丟棄）", async () => {
    ...
    await request(app.app).post("/api/internal/callback-outbox/deliver").send({});
    let entry = (await request(app.app).get(`/api/internal/callback-outbox/${outboxId}`)).body;
    expect(entry.status).toBe("pending");
    expect(entry.attempts).toBe(1);

    // 第 2 次投遞：retry 耗盡（max=2）→ dead_letter，evidence 記錄、不丟棄
    await request(app.app).post("/api/internal/callback-outbox/deliver").send({});
    entry = (await request(app.app).get(`/api/internal/callback-outbox/${outboxId}`)).body;
