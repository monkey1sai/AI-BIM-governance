...
Session / Collaboration Control Plane — review session 協調中心；協調 browser client 與 Kit streaming server 的連線資訊；廣播 presence / selection / annotation 等多人事件；查詢路由 `_bim-control` 與 `_s3_storage`。

埠口：`localhost:8004`（含 Socket.IO）

## MUST

- 所有跨 repo 資料查詢都要走此 service 對外的 REST，不讓 `web-viewer-sample` 直連 `_bim-control` / `_s3_storage`。
...
- ❌ 直接保存大型模型檔案 byte（屬於 `_s3_storage`）。
...
| file URL | **caller**（owner 在 `_s3_storage`） |
